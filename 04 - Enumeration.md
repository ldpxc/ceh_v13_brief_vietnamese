...
- **TCP 25 - SMTP (Simple Mail Transfer Protocol)**: Giao thức gửi thư TCP/IP. Các lệnh thường dùng để liệt kê gồm: `HELO` (Hello), `MAIL FROM` (From), `RCPT TO` (Recipient), `DATA` (Data), `RESET` (Reset), `VRFY` (Verify), `EXPN` (Expand), `HELP` (Help), `QUIT` (Quit).
