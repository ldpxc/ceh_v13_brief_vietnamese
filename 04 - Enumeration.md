...
Liệt kê (Enumeration) là quá trình trích xuất tên người dùng, tên máy, tài nguyên mạng, các chia sẻ và dịch vụ từ một hệ thống hoặc mạng. Trong giai đoạn liệt kê, kẻ tấn công tạo các kết nối tích cực (active connections) với hệ thống và gửi các truy vấn có định hướng để thu thập thêm thông tin về mục tiêu. Sau đó, kẻ tấn công sử dụng thông tin này để xác định các lỗ hổng bảo mật nhằm khai thác hệ thống, từ đó cho phép thực hiện các cuộc tấn công mật khẩu để giành quyền truy cập trái phép.

Lưu ý về pháp lý: Các hoạt động liệt kê có thể là bất hợp pháp tùy thuộc vào chính sách của tổ chức và luật pháp hiện hành. Hacker có đạo đức hoặc người kiểm thử xâm nhập (pen tester) phải luôn xin phép hợp lệ một cách chính thức trước khi thực hiện liệt kê.
...
- **Chia sẻ IPC từ xa:** Trong quá trình liệt kê, kẻ tấn công có thể tình cờ tìm thấy một chia sẻ giao tiếp liên tiến trình (IPC) từ xa, chẳng hạn như IPC$ trong Windows. Chúng có thể thăm dò thêm để kết nối với một chia sẻ quản trị bằng cách brute-force thông tin xác thực admin và lấy toàn bộ thông tin về danh sách tệp hệ thống mà chia sẻ đó đại diện.
...
- **TCP/UDP 53 - DNS Zone Transfer**: Máy chủ DNS sử dụng UDP làm giao thức mặc định, nhưng đối với các truy vấn dài mà UDP thất bại, TCP sẽ được sử dụng làm giải pháp dự phòng. Các phần mềm độc hại (như ADM worm, Bonk Trojan) sử dụng cổng 53 để khai thác lỗ hổng trong DNS.
  - Để thực hiện một cuộc chuyển vùng DNS (DNS zone transfer), kẻ tấn công gửi một yêu cầu chuyển vùng đến máy chủ DNS bằng cách giả vờ là một máy khách; máy chủ DNS sau đó sẽ gửi một phần cơ sở dữ liệu của nó dưới dạng một "zone" cho kẻ tấn công. Vùng này có thể chứa một lượng lớn thông tin về mạng lưới của vùng DNS đó.
...
- **TCP 25 - SMTP (Simple Mail Transfer Protocol)**: Giao thức gửi thư TCP/IP.
  - **HELO <sending-host>**: Khởi tạo phiên giao tiếp với máy chủ.
  - **MAIL FROM:<from-address>**: Chỉ định địa chỉ người gửi.
  - **RCPT TO:<to-address>**: Chỉ định người nhận tin nhắn.
  - **DATA**: Phần dữ liệu (nội dung) của email.
  - **RESET**: Đặt lại kết nối.
  - **VRFY<string>**: Kiểm tra và xác thực xem tên người dùng (địa chỉ) có tồn tại trên máy chủ hay không.
  - **EXPN<string>**: Hiển thị địa chỉ gửi thực tế của các bí danh (aliases) và mở rộng danh sách gửi thư (mailing lists).
  - **HELP[string]**: Yêu cầu hỗ trợ/Trợ giúp.
  - **QUIT**: Thoát.
...
Bước đầu tiên trong việc liệt kê một hệ thống Windows là tận dụng API NetBIOS. NetBIOS ban đầu được phát triển dưới dạng API cho phần mềm máy khách để truy cập tài nguyên mạng cục bộ (LAN). Windows sử dụng NetBIOS để chia sẻ tệp và máy in... Một kẻ tấn công tìm thấy hệ thống Windows có mở cổng 139 có thể kiểm tra xem những tài nguyên nào có thể được truy cập hoặc xem trên hệ thống từ xa. Tuy nhiên, để liệt kê tên NetBIOS, hệ thống từ xa phải được bật tính năng chia sẻ tệp và máy in (file and printer sharing). Tùy thuộc vào tính khả dụng của các chia sẻ, kẻ tấn công có thể đọc/ghi vào hệ thống hoặc khởi chạy tấn công DoS.
...
- **Tiện ích Nbtstat (Nbtstat Utility):** Nbtstat là một tiện ích Windows giúp khắc phục các sự cố phân giải tên NETBIOS. Lệnh nbtstat loại bỏ và sửa chữa các mục đã tải sẵn bằng cách sử dụng các cờ (switches) phân biệt chữ hoa chữ thường. Kẻ tấn công sử dụng Nbtstat để thu thập thống kê giao thức NetBT (NetBIOS over TCP/IP), bảng tên NetBIOS cho cả máy tính cục bộ và từ xa, cũng như bộ nhớ cache tên NetBIOS.
...
- **Công cụ liệt kê NetBIOS bổ sung (NetBIOS Enumeration Tools):**
  ...
  - **Nmap Scripting Engine (NSE) cho NetBIOS**: Kẻ tấn công sử dụng kịch bản NSE `nbstat` để truy xuất tên NetBIOS và địa chỉ MAC của mục tiêu bằng lệnh: `nmap -sV -v --script nbstat.nse <target IP address>`.
...
- **`PsShutdown`** — tắt hoặc khởi động lại máy tính cục bộ hoặc từ xa mà không cần cài đặt phần mềm client.
  - **Cú pháp chi tiết của các công cụ PsTools:**
    - **PsExec:** `psexec [\\computer[,computer2[,...] | @file]][-u user [-p psswd] [-n s][-r servicename] [-h] [-l] [-s|-e] [-x] [-i [session]][-c executable [-f|-v]][-w directory] [-d] [-<priority>][-a n,n,...] cmd [arguments]`
    - **PsFile:** `psfile [\\RemoteComputer [-u Username [-p Password]]] [[Id | path] [-c]]`
    - **PsGetSid:** `psgetsid [\\computer[,computer[,...] | @file] [-u username [-p password]]] [account|SID]`
    - **PsKill:** `pskill [- ] [-t] [\\computer [-u username] [-p password]] <process name | process id>`
    - **PsInfo:** `psinfo [[\\computer[,computer[,..] | @file [-u user [-p psswd]]] [-h] [-s] [-d] [-c [-t delimiter]] [filter]`
    - **PsLoggedOn:** `psloggedon [- ] [-l] [-x] [\\computername | username]`
    - **PsLogList:** `psloglist [- ] [\\computer[,computer[,...] | @file [-u username [-p password]]] [-s [-t delimiter]] [-m #|-n #|-h #|-d #|-w] [-c] [-x] [-r] [-a mm/dd/yy] [-b mm/dd/yy] [-f filter] [-i ID[,ID[,...] | -e ID[,ID[,...]]] [-o event source[,event source][,..]]] [-q event source[,event source][,..]]] [-l event log file] <eventlog>`
    - **PsPasswd:** `pspasswd [[\\computer[,computer[,..] | @file [-u user [-p psswd]]] Username [NewPassword]`
    - **PsShutdown:** `psshutdown [[\\computer[,computer[,..] | @file [-u user [-p psswd]]] -s|-r|-h|-d|-k|-a|-l|-o [-f] [-c] [-t nn|h:m] [-n s] [-v nn] [-e [u|p]:xx:yy] [-m "message"]`
...
- **`snmpwalk`**: Công cụ dòng lệnh quét các node SNMP. Lệnh thường dùng:
  ...
  - `snmpwalk -v2c -c public <Target IP Address> hrSWInstalledName` (Tìm kiếm phần mềm đã cài đặt)
  - `snmpwalk -v2c -c public <Target IP Address> hrMemorySize` (Xác định dung lượng RAM trên host)
  - `snmpwalk -v2c -c public <Target IP Address> <OID> <New Value>` (Thay đổi một OID sang giá trị khác)
  - `snmpwalk -v2c -c public <Target IP Address> sysContact <New Value>` (Thay đổi OID sysContact)
- **Nmap**: `nmap -sU -p 161 --script=snmp-processes <Target IP Address>`
  - `nmap -sU -p 161 --script=snmp-sysdescr <Target IP Address>` (Lấy thông tin về loại máy chủ SNMP và chi tiết hệ điều hành)
  - `nmap -sU -p 161 --script=snmp-win32-software <Target IP Address>` (Lấy danh sách tất cả các ứng dụng đang chạy trên máy mục tiêu)
...
Để hoàn thành nhiệm vụ chia sẻ tệp và thư mục qua mạng, quy trình "xuất" (exporting) được sử dụng. Tuy nhiên, máy khách trước tiên sẽ cố gắng làm cho tệp có sẵn để chia sẻ bằng cách sử dụng quy trình "gắn kết" (mounting). Vị trí /etc/exports trên máy chủ NFS chứa danh sách các máy khách được phép chia sẻ tệp trên máy chủ. Trong cách tiếp cận này, để truy cập máy chủ, thông tin xác thực duy nhất được sử dụng là địa chỉ IP của máy khách.
...
- **Công cụ và Lệnh**:
  ...
  - **SuperEnum bao gồm một tập lệnh thực hiện quá trình liệt kê cơ bản mọi cổng mở. Kẻ tấn công sử dụng tập lệnh ./superenum và sau đó nhập tên tệp văn bản (ví dụ: Target.txt) có chứa địa chỉ IP mục tiêu hoặc danh sách các địa chỉ IP cần liệt kê.**
...
- **Công cụ và kỹ thuật**:
  ...
  - **Nmap** (Các script mở rộng):
    - Liệt kê người dùng: `nmap -p 25 --script=smtp-enum-users <Target IP>`
    - Liệt kê tất cả các lệnh SMTP có sẵn: `nmap -p 25, 365, 587 --script=smtp-commands <Target IP Address>`
    - Xác định các Open Relays của SMTP: `nmap -p 25 --script=smtp-open-relay <Target IP Address>`
...
- **Svmap**: Scanner nguồn mở nhận diện thiết bị SIP và máy chủ PBX. Lệnh: `svmap <target network range/IP Address>`.
  - **Các tính năng của Svmap:**
    - Xác định các thiết bị SIP và máy chủ PBX trên các cổng mặc định và không mặc định.
    - Quét các dải mạng lớn (Scan large ranges of networks).
    - Quét một máy chủ trên nhiều cổng khác nhau để tìm dịch vụ SIP hoặc quét nhiều máy chủ trên nhiều cổng.
    - Đổ chuông tất cả các điện thoại trên mạng cùng một lúc bằng phương thức `INVITE`.
...
- **`finger`**: Hiển thị tên đăng nhập, thư mục home, thời gian nhàn rỗi (idle), sdt... Lệnh: `finger [-l] [-m] [-p] [-s] [user ...] [user@host ...]`.
  - **Các tùy chọn của lệnh finger:**
    - `-s`: Hiển thị tên đăng nhập, tên thật, tên thiết bị đầu cuối (terminal), thời gian nhàn rỗi (idle), thời gian đăng nhập, vị trí văn phòng và số điện thoại văn phòng.
    - `-l`: Tạo định dạng nhiều dòng hiển thị tất cả thông tin như tùy chọn `-s` cùng với thư mục home, số điện thoại nhà riêng, login shell, trạng thái mail, và nội dung các tệp `.plan`, `.project`, `.pgpkey`, `.forward` từ thư mục home của người dùng.
    - `-p`: Ngăn tùy chọn `-l` hiển thị nội dung của các tệp `.plan`, `.project` và `.pgpkey`.
    - `-m`: Ngăn chặn việc khớp (matching) các tên người dùng.
...
- AI sẽ tạo ra một file script Bash (network_enumeration.sh) để tự động hóa:
- Lệnh thực thi script: `chmod +x network_enumeration.sh && ./network_enumeration.sh`
...
- Sửa đổi registry để chỉ cho phép các truy cập bị hạn chế hoặc được phép đối với tên cộng đồng (community name) SNMP.
- Cấu hình người dùng SNMPv3 trong cụm (cluster) để tăng cường bảo mật bằng xác thực và mã hóa.
...
- Đảm bảo rằng netgroup NIS có tên máy chủ (hostname) được định nghĩa đầy đủ để ngăn chặn việc vô tình cấp quyền truy cập cao hơn cho các máy chủ khác.
- Giữ các máy chủ và máy khách NFS trong một phân đoạn mạng an toàn, được phân lập để giới hạn quyền truy cập từ các phân đoạn mạng không được phép.
...
- Đảm bảo rằng quy ước SMB đối mặt với web (web confronting) và các máy chủ DNS chính (DNS mainframes) đã bị vô hiệu hóa.
- Cấu hình máy chủ SMB để ghi log (nhật ký) các nỗ lực truy cập và những thay đổi đối với tài nguyên chia sẻ. Thường xuyên xem xét log để phát hiện hoạt động đáng ngờ.
