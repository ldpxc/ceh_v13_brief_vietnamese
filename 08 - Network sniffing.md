...
- _(Bổ sung thêm các công cụ từ nguồn)_: PTW60, P5551A PCIe 5.0 Protocol Exerciser, Voyager M4x, N2X N5540A Agilent Protocol Analyzer, Xgig 16-Lane PCI Express 4.0 Chassis.
...
- **Reconfigure:** Máy chủ thông báo cho client rằng nó có các cài đặt cấu hình mới hoặc được cập nhật. Client sau đó sẽ gửi yêu cầu gia hạn/trả lời để nhận thông tin.
- **Relay-Forward:** Relay agent gửi thông điệp chuyển tiếp (relay-forward) để đưa gói tin tới các máy chủ (trực tiếp hoặc qua một relay agent khác).
- **Relay-Reply:** Máy chủ gửi thông điệp trả lời (relay-reply) tới relay agent, chứa thông tin để agent này giao lại cho client.
...
- **Hardware Address Type (1 byte):** Loại địa chỉ phần cứng do IANA định nghĩa (Ví dụ "1" = 10 Mb Ethernet).
- **Hardware Address Length (1 byte):** Độ dài địa chỉ phần cứng (tính bằng octet).
- **Hops (1 byte):** Thường được client gán bằng "0"; tuy nhiên tùy chọn này dùng để đếm số lượng relay agents mà thông điệp đã đi qua.
- **Seconds (2 bytes):** Số giây trôi qua kể từ khi client bắt đầu quá trình xin cấp IP hoặc gia hạn.
- **Flags (2 bytes):** Các cờ do client thiết lập (ví dụ nếu client không thể nhận datagram unicast IP, cờ broadcast sẽ được bật).
- **Server IP Address (SIADDR) (4 bytes):** IP của máy chủ DHCP.
- **Gateway IP Address (GIADDR) (4 bytes):** IP của DHCP relay agent.
- **Server Name (SNAME) (64 bytes):** Tên hostname của máy chủ (Tùy chọn).
- **File Name (128 bytes):** Tên file chứa boot image của BOOTP client.
- **DHCP Options (Variable):** Các tùy chọn cấu hình bổ sung của DHCP.
