### CEHv13 - Module 10 - Denial-of-Service
...
- **Mục tiêu của DoS:** Ngăn chặn người dùng hợp pháp sử dụng hệ thống, thay vì giành quyền truy cập trái phép hoặc làm hỏng dữ liệu. Các hình thức bao gồm: Tiêu thụ băng thông/tài nguyên, phá hủy vật lý các thành phần mạng, hoặc làm hỏng cấu trúc lập trình/tệp tin.
  - **Các ví dụ về các loại hình tấn công DoS (Examples for types of DoS attacks):**
    ...
    - _Ví dụ minh họa:_ Hãy xem xét một công ty dịch vụ ăn uống chủ yếu nhận đơn hàng qua điện thoại. Nếu một kẻ tấn công muốn phá vỡ hoạt động kinh doanh này, chúng sẽ tìm cách chặn đường dây điện thoại, khiến công ty không thể làm việc. Tấn công DoS cũng hoạt động theo cơ chế tương tự—kẻ tấn công sử dụng hết mọi cách để kết nối với hệ thống nạn nhân, khiến các hoạt động kinh doanh hợp pháp trở nên bất khả thi.
    - Các cuộc tấn công DoS thường không dẫn đến việc đánh cắp thông tin. Tuy nhiên, trong trường hợp xấu nhất, tấn công DoS có thể gây ra sự cố phá hủy ngẫu nhiên các tệp và chương trình của hàng triệu người dùng đang kết nối với hệ thống nạn nhân tại thời điểm bị tấn công.
...
- **Tấn công DDoS (Distributed Denial-of-Service):** Là một cuộc tấn công DoS quy mô lớn, được phối hợp và khởi chạy gián tiếp thông qua nhiều máy tính bị xâm nhập (botnets/zombies). Máy chủ bị tấn công gọi là "nạn nhân chính" (primary victim), còn các máy bị xâm nhập dùng để tấn công gọi là "nạn nhân phụ" (secondary victims).
  - Theo định nghĩa từ _Câu hỏi thường gặp về Bảo mật World Wide Web (WWWS FAQ)_: "Kẻ tấn công nhân lên hiệu quả của việc từ chối dịch vụ một cách đáng kể bằng cách khai thác tài nguyên của nhiều máy tính đồng lõa vô tình (unwitting accomplice computers), đóng vai trò là nền tảng tấn công." Các cuộc tấn công DDoS trở nên phổ biến vì dễ dàng tiếp cận các kế hoạch khai thác và cần rất ít nỗ lực trí tuệ để thực thi.
...
- Các tổ chức tội phạm này tạo và cho thuê mạng botnet, đồng thời cung cấp nhiều dịch vụ đa dạng từ phát triển phần mềm độc hại, hack tài khoản ngân hàng đến triển khai các cuộc tấn công DoS khổng lồ chống lại bất kỳ mục tiêu nào với một mức giá nhất định. Ví dụ: một tổ chức tội phạm có thể thực hiện tấn công DDoS vào ngân hàng để đánh lạc hướng sự chú ý của đội ngũ bảo mật trong khi chúng dọn sạch tài khoản ngân hàng bằng thông tin đăng nhập bị đánh cắp. Sự tham gia ngày càng tăng của chúng vào các cuộc chiến tranh mạng mang động cơ chính trị đang là vấn đề đáng lo ngại.
...
- Sự ra đời của botnet dẫn đến sự gia tăng to lớn về tội phạm mạng. Botnet tạo thành cốt lõi của trung tâm hoạt động tội phạm mạng liên kết và hợp nhất nhiều bộ phận khác nhau trong thế giới ngầm. Một botnet tương đối nhỏ gồm 1.000 bot có tổng băng thông lớn hơn băng thông của hầu hết các hệ thống doanh nghiệp (corporate systems).
...
- _(Các kỹ thuật tiêu biểu)_: HTTP GET/POST attack, Slowloris (chỉ gửi yêu cầu HTTP một phần/nửa mở để giữ kết nối), UDP application layer flood, DDoS extortion attack (Tống tiền bằng DDoS).
  - Trong một số trường hợp, thông qua tràn bộ đệm (buffer overflow), kẻ tấn công còn có thể thực thi mã tùy ý (arbitrary code) trên hệ thống từ xa. Việc gửi quá nhiều dữ liệu đến một ứng dụng sẽ ghi đè lên dữ liệu kiểm soát chương trình, cho phép hacker chạy mã của chính họ thay thế.
  - Bởi vì kết nối được thiết lập hợp lệ và lưu lượng truy cập đi vào mục tiêu trông có vẻ bình thường, nên rất khó để phát hiện các cuộc tấn công này. Tuy nhiên, nếu người dùng nhận diện được nó, họ có thể ngăn chặn và truy tìm ngược về nguồn gốc (trace it back to its source) dễ dàng hơn nhiều so với các loại tấn công DDoS khác.
...
- **Ping of Death (PoD)** [Trang 1492]: Kẻ tấn công gửi gói tin malformed hoặc vượt kích thước hợp lệ (oversized packets) bằng lệnh ping đơn giản. Ví dụ: gửi gói 65.538 byte, vượt quá giới hạn 65.535 byte do chuẩn RFC 791 IP quy định. Quá trình lắp ráp lại (reassembly) tại máy đích sẽ gây sập hệ thống (crash).
  - Trong cuộc tấn công này, danh tính của kẻ tấn công có thể dễ dàng bị giả mạo và chúng có thể không cần kiến thức chi tiết về máy mục tiêu, ngoại trừ địa chỉ IP của máy đó.
...
- **Pulse-wave DDoS** [Trang 1493]: Gửi các xung gói tin theo chu kỳ (ví dụ: các đợt bùng nổ traffic cực lớn cách nhau mỗi 10 phút, mỗi xung có thể đạt 300 Gbps hoặc hơn). Mục đích là làm hệ thống kiệt quệ băng thông toàn bộ và khiến việc phục hồi vô cùng khó khăn.
  - Việc phục hồi từ các cuộc tấn công (Pulse-wave) như vậy là rất khó khăn và đôi khi là không thể.
- **Zero-day DDoS** [Trang 1493]: Khai thác các lỗ hổng DoS chưa có bản vá hoặc chưa có cơ chế phòng thủ hiệu quả. Kẻ tấn công chủ động chặn tài nguyên và đánh cắp dữ liệu trước khi nạn nhân kịp nhận ra và tung ra bản vá.
  - Hiện tại, không có phương pháp tiếp cận đa năng (versatile approach) nào để bảo vệ toàn diện mạng lưới khỏi loại hình tấn công Zero-day DDoS này.
...
- **SYN flood (SYN flood attack)** [Trang 1496 - 1498]: Lạm dụng cơ chế bắt tay 3 bước (three-way handshake) của TCP. Kẻ tấn công gửi lượng lớn SYN với IP nguồn giả. Máy chủ trả lời bằng SYN/ACK và chờ gói ACK cuối cùng (lưu trong "listen queue" ít nhất 75 giây). Hàng đợi đầy khiến máy chủ không thể nhận kết nối mới.
  - _(Bổ sung từ nguồn)_ Có các biến thể khác như SYN-ACK Flood và ACK / PUSH ACK Flood.
    - ACK and PUSH ACK Flood Attack: Trong một phiên TCP đang hoạt động, cờ ACK và PUSH ACK được sử dụng để truyền thông tin qua lại giữa máy chủ và máy khách cho đến khi phiên kết thúc. Kẻ tấn công gửi một lượng lớn các gói ACK và PUSH ACK giả mạo đến máy mục tiêu, khiến nó trở nên không thể hoạt động (non-functional).
...
- **Fragmentation attack (Tấn công phân mảnh)** [Trang 1499]: Gửi lượng lớn gói tin bị phân mảnh (ví dụ gói ~1500 byte) với nội dung ngẫu nhiên khiến nạn nhân cạn kiệt tài nguyên khi cố ghép lại. Vì giao thức cho phép phân mảnh, kỹ thuật này thường dễ dàng qua mặt thiết bị mạng, firewall, và IDS/IPS.
  - Do giao thức IP vốn cho phép phân mảnh, các gói tin này thường đi qua các thiết bị mạng như bộ định tuyến, tường lửa và IDS/IPS mà không bị kiểm tra. Việc lắp ráp và kiểm tra lại tiêu tốn quá nhiều tài nguyên, đặc biệt khi nội dung gói tin bị kẻ tấn công xáo trộn một cách ngẫu nhiên.
...
- **Slowloris** [Trang 1501 - 1503]: Kẻ tấn công gửi các yêu cầu HTTP một phần (không hoàn chỉnh) để giữ cho nhiều kết nối ở trạng thái nửa mở (half-open). Máy chủ chờ đợi đến khi cạn kiệt nhóm kết nối đồng thời (concurrent connection pool).
  - Công cụ này hoàn toàn khác biệt so với các công cụ DDoS khác ở chỗ nó sử dụng lưu lượng HTTP hoàn toàn hợp lệ (perfectly legitimate HTTP traffic) để đánh sập máy chủ mục tiêu.
...
- **Permanent Denial-of-Service (PDoS / Phlashing)** [Trang 1507]: Gây hỏng phần cứng vĩnh viễn (gọi là "bricking a system"). Kẻ tấn công gửi bản cập nhật firmware giả mạo, chứa lỗi (qua email, mạng xã hội, video). Khi nạn nhân cài đặt, thiết bị mạng (router, máy in) bị phá hủy hoặc bị chiếm quyền điều khiển từ xa hoàn toàn.
  - Loại tấn công PDoS này nhanh hơn và mang tính phá hoại cao hơn so với tấn công DoS thông thường. Nó hoạt động với một lượng tài nguyên hạn chế, không giống như DDoS cần phải giải phóng một "đội quân zombie" lớn lên mục tiêu.
...
- **Distributed Reflection DDoS (DRDoS / Spoofed attack)** [Trang 1509 - 1510]: Tấn công giả mạo nguồn thông qua hàng loạt máy trung gian (zombies) và máy phản xạ (reflectors). Gửi TCP SYN (với IP nguồn là máy nạn nhân) tới reflector, reflector tin rằng nạn nhân yêu cầu kết nối nên đồng loạt dội ngược gói SYN/ACK về phía nạn nhân với băng thông khổng lồ.
  - Đây là một cuộc tấn công thông minh vì rất khó hoặc thậm chí không thể truy tìm kẻ tấn công. DRDoS hiệu quả hơn một cuộc tấn công DDoS điển hình vì nhiều nạn nhân trung gian (intermediary) và phản xạ (secondary victims) cùng tạo ra băng thông tấn công khổng lồ dội về đích.
...
- **UltraDDOS-v2:** Cung cấp giao diện đồ họa (GUI) đơn giản để kẻ tấn công nhập IP đích, số cổng và số lượng gói tin muốn truyền đi.
...
### 7.1. Các chiến lược đối phó DoS/DDoS (DoS/DDoS Countermeasure Strategies) [Trang 1518]

- Hấp thụ cuộc tấn công (Absorbing the Attack): Sử dụng dung lượng hệ thống bổ sung để hấp thụ cuộc tấn công. Chiến lược này yêu cầu lập kế hoạch trước và tài nguyên bổ sung. Nhược điểm lớn là chi phí cho các tài nguyên tăng thêm sẽ bị lãng phí ngay cả khi không có cuộc tấn công nào đang diễn ra.
- Làm suy giảm dịch vụ (Degrading Services): Nếu không thể duy trì mọi dịch vụ trong cuộc tấn công, cần xác định các dịch vụ cực kỳ quan trọng (critical services). Sau đó, thiết kế mạng và ứng dụng được tùy chỉnh để cắt giảm hoặc tắt các dịch vụ không quan trọng, tập trung tài nguyên duy trì chức năng cốt lõi.
- Tắt toàn bộ dịch vụ (Shutting Down Services): Tắt tất cả các dịch vụ cho đến khi cuộc tấn công lắng xuống hoàn toàn. Dù không phải là lựa chọn lý tưởng, đây có thể là phản ứng hợp lý và an toàn nhất trong một số trường hợp.

### 7.2. Các kỹ thuật phát hiện (Detection Techniques) [Trang 1516 - 1517]
...
### 7.3. Các chiến lược giảm nhẹ & phòng thủ (Mitigation Strategies) [Trang 1518, 1521, 1525]
...
### 7.4. Ngăn chặn các cuộc tấn công tiềm ẩn (Prevent Potential Attacks) [Trang 1521 - 1522]

- Egress Filtering (Lọc dữ liệu đi ra): Quét các tiêu đề (headers) của gói IP rời khỏi mạng. Đảm bảo rằng lưu lượng độc hại hoặc chưa được xác thực không bao giờ rời khỏi mạng nội bộ. Ví dụ, nếu máy chủ bị khai thác lỗ hổng zero-day, việc bật Egress Filtering sẽ ngăn máy chủ đó thiết lập kết nối ngược lại (connection back) với kẻ tấn công.
- Ingress Filtering (Lọc dữ liệu đi vào): Kỹ thuật lọc gói tin được nhiều ISP sử dụng để ngăn chặn việc giả mạo địa chỉ nguồn của lưu lượng Internet. Nó bảo vệ chống lại các cuộc tấn công tràn ngập bắt nguồn từ các tiền tố IP hợp lệ và cho phép truy tìm nguồn gốc thực sự (true source) của kẻ tấn công.

### 7.5. Giảm nhẹ tấn công (Mitigate Attacks) [Trang 1525 - 1526]

- Cân bằng tải (Load Balancing): Tăng băng thông trên các kết nối quan trọng để hấp thụ lưu lượng độc hại. Sử dụng mô hình máy chủ sao chép (replicated server model) trong kiến trúc nhiều máy chủ giúp cân bằng tải tốt hơn, vừa tăng hiệu suất mạng thông thường vừa giảm thiểu thiệt hại từ DDoS.
- Điều tiết (Throttling): Thiết lập bộ định tuyến với logic "min-max fair server-centric" để điều tiết mức lưu lượng truy cập đi vào ở mức an toàn cho máy chủ. Nó giúp lọc lưu lượng người dùng hợp lệ khỏi lưu lượng giả mạo. Nhược điểm: Đôi khi kích hoạt báo động giả (false alarms) và loại bỏ nhầm lưu lượng hợp lệ.
- Loại bỏ yêu cầu (Drop Requests): Khi tải tăng lên, hệ thống sẽ yêu cầu người gửi phải giải một "câu đố khó" (difficult puzzle) đòi hỏi sức mạnh tính toán hoặc bộ nhớ (ví dụ như CAPTCHA ngầm) trước khi tiếp tục. Khi gặp yêu cầu này, máy móc của mạng zombie sẽ bị suy giảm hiệu suất và thường từ bỏ việc truyền tải tấn công DDoS.

### 7.6. Kỹ thuật phòng thủ chống lại Botnet (Techniques to Defend against Botnets) [Trang 1529 - 1530]
...
### 7.7. Các biện pháp phòng ngừa DoS/DDoS bổ sung [Trang 1530 - 1531]
...
### 7.8. Ghi nhận pháp y sau tấn công (Post-Attack Forensics) [Trang 1527]
