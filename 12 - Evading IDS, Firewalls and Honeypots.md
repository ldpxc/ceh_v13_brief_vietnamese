...
- **CPL side-loading** [Trang 1742 - 1744]: Giả mạo applet của Control Panel (file `.cpl`). Đóng gói shellcode bằng Base64 và nhúng vào file CPL để qua mặt AV. Công cụ: `CPLResourceRunner`.
  - **Cấu trúc mã C cho file .cpl:**
    ...
    #include <stdio.h>
    #include <Windows.h>
    BOOL APIENTRY DLLMain(HMODULE hModule, DWORD ul_reason_for_call_LPVOID IpReserved) {
        //malicious code
...
- **Timing-based evasion:** Kỹ thuật né tránh hộp cát (sandbox evasion) bằng cách thiết lập mã độc chỉ thực thi vào một thời điểm cụ thể hoặc sau các thao tác nhất định của nạn nhân (như click chuột hoặc khởi động lại hệ thống). Các ví dụ gồm: sleep patching, delay APIs, và time bombs.
...
- **Phân tích thời gian phản hồi (Analyzing Response Time):** Honeypots thường có độ trễ cao hoặc không nhất quán do phải xử lý thêm các lớp ghi log và giám sát. Đo lường bằng Ping, Traceroute hoặc lệnh Nmap (`nmap -p 80 --scan-delay 1s --max-retries 5 target_ip`).
  - *Ví dụ lệnh kiểm tra độ trễ bằng Nmap:* `nmap -p 80 --scan-delay 1s --max-retries 5 <target_ip>`.
